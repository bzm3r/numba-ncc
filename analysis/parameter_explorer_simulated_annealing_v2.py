...
def calculate_energy(TOTAL_TIME, task_index, task_defn_dict):
    cell = run_environment_dynamics(TOTAL_TIME, task_index, task_defn_dict)
    displacement = analysis.calculate_total_displacement(cell)
    average_rac_activity, average_rho_activity = analysis.calculate_average_rgtpase_activity(cell)
    symmetry_values = analysis.score_symmetry(cell).values()
    
    if average_rac_activity < 0.4:
        rac_activity_score = 0
    else:
        rac_activity_score = (10*(0.4 - average_rac_activity))**4
        
    if average_rho_activity < 0.4:
        rho_activity_score = 0
    else:
        rho_activity_score = (10*(0.4 - average_rho_activity))**4
        
    
    score = (1/(displacement/(TOTAL_TIME/60.0))) + rac_activity_score + rho_activity_score + 1/symmetry_values[0] + 1/symmetry_values[1]
...
def acceptance_probability_function(energy_alpha, task_defn_dict_beta, temperature, TOTAL_TIME, task_index=-1):
    displacement_beta, energy_beta = calculate_energy(TOTAL_TIME, task_index, task_defn_dict_beta)
    
    if energy_beta < energy_alpha:
        return displacement_beta, energy_beta, 1
    else:
        return displacement_beta, energy_beta, np.exp(-(energy_beta - energy_alpha)/temperature)
        
# -------------------------------------
        
def generate_possible_transition(task_defn_dict, num_parameters, parameter_exploration_program):
#    num_tweaks = 1
#    #num_tweaks = np.random.randint(1, high=num_parameters+1)
    transition_dict = copy.deepcopy(task_defn_dict)
    
    parameter_indices = np.arange(num_parameters)
    for parameter_index in parameter_indices:
        label, min_val, max_val, possible_delta = parameter_exploration_program[parameter_index]
        current_val = task_defn_dict[label]
        
        delta_sign = np.random.randint(-1, high=2)
        ...
        if delta_sign < 0:
            delta_mag = current_val - min_val
        else:
            delta_mag = max_val - current_val
            
            transition_dict[label] += delta_sign*np.random.rand()*delta_mag
...
g_trial_list = []
def parameter_explorer_asymmetry_criteria(parameter_exploration_program, TOTAL_TIME=500, init_temp_mag=20, cooldown_rate=1, max_tries=1000):
    ...
    max_time = np.round((np.log(1e-2) - np.log(init_temp_mag))/(-1*cooldown_rate), decimals=2)
    
    print "max_time: ", max_time
    
    print "setting up times and tempeartures..."
    times = np.linspace(0, max_time, num=max_tries)
    temperatures = init_temp_mag*np.exp(-1*cooldown_rate*times)
    ...
    current_displacement, current_energy = calculate_energy(TOTAL_TIME, -1, task_defn_dict)

    global g_trial_list
    g_trial_list += [(current_energy, task_defn_dict)]
    len_trial_list = 1
    time_takens = []
    energy_ratings = [current_energy]
    ...
    for try_number in range(max_tries):
        ...
        if np.random.rand() <= acceptance_probability:
            ...
            current_displacement = symmetry_next
            current_energy = energy_next
            g_trial_list.append((current_energy, current_displacement, task_defn_dict))
            len_trial_list += 1
            energy_ratings.append(current_energy)
        else:
            print "Not accepted."
...
#    results = parameter_explorer_asymmetry_criteria([('kgtp_rac_multiplier', 50, 150, 5), ('kgtp_rho_multiplier', 50, 150, 5), ('kgtp_rho_autoact_multiplier', 100, 250, 5),('kgtp_rac_autoact_multiplier', 100, 250, 5), ('kdgtp_rac_multiplier', 20, 20, 1), ('kdgtp_rho_multiplier', 20, 20, 1), ('kdgtp_rac_mediated_rho_inhib_multiplier', 100, 250, 5), ('kdgtp_rho_mediated_rac_inhib_multiplier', 100, 250, 5), ('threshold_rac_autoact_multiplier', 0.5, 0.5, 1), ('threshold_rho_autoact_multiplier', 0.5, 0.5, 1), ('threshold_rho_mediated_rac_inhib_multiplier', 0.5, 0.5, 1), ('threshold_rac_mediated_rho_inhib_multiplier', 0.5, 0.5, 1), ('threshold_rac_autoact_dgdi_multiplier', 0.5, 0.5, 1), ('stiffness_edge', 3e-10, 3e-10, 1)], sequential=False, min_symmetry_score=0, TOTAL_TIME=500)
    ...
    parameter_explorer_asymmetry_criteria([('kgtp_rac_multiplier', 20, 75, 5), ('kgtp_rho_multiplier', 20, 100, 5), ('kgtp_rho_autoact_multiplier', 100, 250, 5),('kgtp_rac_autoact_multiplier', 100, 250, 5), ('kdgtp_rac_multiplier', 15, 40, 5), ('kdgtp_rho_multiplier', 15, 40, 5), ('kdgtp_rac_mediated_rho_inhib_multiplier', 100, 250, 5), ('kdgtp_rho_mediated_rac_inhib_multiplier', 100, 250, 5                                                                             ), ('threshold_rac_autoact_multiplier', 0.25, 1.25, 0.05), ('threshold_rho_autoact_multiplier', 0.25, 1.25, 0.05), ('threshold_rho_mediated_rac_inhib_multiplier', 0.25, 1.25, 0.05), ('threshold_rac_mediated_rho_inhib_multiplier', 0.25, 1.25, 0.05), ('threshold_rac_autoact_dgdi_multiplier', 0.25, 1.25, 0.05)], cooldown_rate=0.025, TOTAL_TIME=500, init_temp_mag=50, max_tries=1000)
    ...
    sorted_results = sorted(g_trial_list, key = lambda x: x[0])
        
    plt.plot([x[0] for x in g_trial_list], 'b.')
