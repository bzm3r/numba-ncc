...
# ----------------------------------------------------------------------------------------
@nb.jit(nopython=True)
def eulerint(current_state, tpoints, args, num_int_steps):
    num_tpoint_pairs = tpoints.shape[0] - 2
    ...
    for i in range(tpoint_pairs.shape[0]):
        ...
        for j in range(num_int_steps):
            current_state = current_state + dt*cell_dynamics_nopython(current_state, *args)
