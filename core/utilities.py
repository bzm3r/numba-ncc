...
def calculate_parameter_exploration_score_from_cell_no_randomization_variant_old(a_cell, should_be_polarized_by_in_hours=0.5):
    T = a_cell.T
...
def calculate_total_displacement(num_nodes, cell_index, storefile_path):
    ...
    init_centroid = geometry.calculate_centroid(init_node_coords)
    final_centroid = geometry.calculate_centroid(final_node_coords)
...
def calculate_sum_displacement_per_interval(num_nodes, num_timesteps, cell_index, num_timesteps_to_pick, storefile_path):
    ...
    centroids_of_interest = np.array([geometry.calculate_centroid(x) for x in ncs_of_interest])
...
def calculate_average_total_strain(num_nodes, cell_index, storefile_path):
    ...
    init_perimeter = geometry.calculate_perimeter(node_coords_per_timestep[0])
    ...
    for pi in range(num_protrusions):
        ...
        if ti_start == 0:
            ...
            if last_protrusion_at_this_node_end_tstep != -1:
                start_buffer_a, start_buffer_b = last_protrusion_at_this_node_end_tstep + int(0.75*(ti_start - last_protrusion_at_this_node_end_tstep)), ti_start
            else:
            ...
            average_randomization_factor = np.average(normalized_randomization_factors_per_tstep[start_buffer_a:(start_buffer_b+1), ni])
            ...
            if not start_cause_found:
                ...
                p1_neigher_node_rac = normalized_rac_membrane_active_per_tstep[start_buffer_a:(start_buffer_b+1), (ni+1)%num_nodes]
                m1_neigher_node_rac = normalized_rac_membrane_active_per_tstep[start_buffer_a:(start_buffer_b+1), (ni-1)%num_nodes]
        ...
        average_cil_signal = calculate_average_cil_signal(ni, normalized_cil_signals_per_tstep, end_buffer_a, end_buffer_b)
...
def collate_protrusion_data(num_cells, T, storefile_path, max_tstep=None):
    ...
    for cell_index in range(num_cells):
        protrusion_data_per_cell.append(collate_protrusion_data_for_cell(cell_index, T, storefile_path, max_tstep=max_tstep))
...
def calculate_all_cell_speeds_and_directions_until_tstep(num_cells, max_tstep, storefile_path, T, cell_Ls):
    ...
    for ci in range(num_cells):
        all_cells_speed_and_directions.append(calculate_cell_speeds_and_directions_until_tstep(ci, max_tstep, storefile_path, T, cell_Ls[ci]))
...
def calculate_normalized_group_area_over_time(num_cells, num_timepoints, storefile_path):
    ...
    if num_cells == 0:
        ...
        distance_between_cells_at_all_timesteps = np.linalg.norm(all_cell_centroids_per_tstep[:, 0, :] - all_cell_centroids_per_tstep[:, 1, :], axis=1)
...
#@nb.jit(nopython=True)
def calculate_simple_intercellular_separations_and_subgroups_per_timestep(init_cell_group_separation, all_cell_centroids_per_tstep):
    ...
    for ti in range(num_timepoints):
        ...
        intercellular_separations_per_timestep[ti], subgroups = calculate_simple_intercellular_separations_and_subgroups(init_cell_group_separation, relevant_cell_centroids)
...
def calculate_normalized_group_area_and_average_cell_separation_over_time(cell_radius, num_cells, num_timepoints, storefile_path, get_subgroups=False):
    ...
    if num_cells == 0:
        return np.zeros(num_timepoints, dtype=np.float64), np.zeros(num_timepoints, dtype=np.float64), cell_subgroups_per_timestep
    elif num_cells == 1:
        return np.ones(num_timepoints, dtype=np.float64), np.ones(num_timepoints, dtype=np.float64), None
    elif num_cells == 2:
        distance_between_cells_at_all_timesteps = np.linalg.norm(all_cell_centroids_per_tstep[:, 0, :] - all_cell_centroids_per_tstep[:, 1, :], axis=1)
        
        return np.nan*np.zeros(num_timepoints, dtype=np.float64), distance_between_cells_at_all_timesteps/distance_between_cells_at_all_timesteps[0], cell_subgroups_per_timestep
    else:
        delaunay_triangulations_per_tstep = []
        ...
        for ti, all_cell_centroids in enumerate(all_cell_centroids_per_tstep):
            ...
            if dt != None:
                dt = delaunay_triangulations_per_tstep[ti]
                simplices = all_cell_centroids[dt.simplices]
                simplex_areas = np.array([geometry.calculate_polygon_area(simplex) for simplex in simplices])
                convex_hull_areas_per_tstep.append(np.round(np.sum(simplex_areas), decimals=3))
        ...
        return np.array(convex_hull_areas_per_tstep)/convex_hull_areas_per_tstep[0], np.array(average_cell_separation_per_tstep)/min_cell_separation_tstep0, cell_subgroups_per_timestep
