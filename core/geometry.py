...
def create_initial_bounding_box_polygon_array(num_cells, num_nodes_per_cell, environment_cells_node_coords):
    ...
    for ci in range(num_cells):
        bounding_box_polygon_array[ci] = calculate_bounding_box_polygon(environment_cells_node_coords[ci])
...
def are_points_inside_polygon(num_points, points, num_poly_vertices, polygon):
    ...
    for index in range(num_points):
        ...
        if is_point_in_polygon_given_bounding_box(test_point, min_x, max_x, min_y, max_y):
            results[index] = is_point_in_polygon_without_bb_check(test_point, num_poly_vertices, polygon)
    ...
    for vi in range(num_vertices):
        ...
        if ignore_vertex_index != -1:
            if vi == ignore_vertex_index:
                continue
            elif next_index == ignore_vertex_index:
                continue
    ...
    if check_if_line_segment_intersects_polygon(coords_a, coords_b, normal_to_line_segment, all_polygon_coords[pi_a], vi_a):
        ...
        for pi in range(num_polygons):
            if pi == pi_a or pi == pi_b:
                ...
                this_poly_bounding_box = all_polygons_bounding_box_coords[pi]
                if check_if_line_segment_intersects_polygon(coords_a, coords_b, normal_to_line_segment, this_poly_bounding_box, -1):
