...
# -----------------------------------------------------------------
@nb.jit(nopython=True)         
def calculate_phys_space_bdry_contact_factors(num_vertices, this_cell_coords, space_physical_bdry_polygons):
...
# -----------------------------------------------------------------
@nb.jit(nopython=True)    
def calculate_migr_bdry_contact_factors(this_cell_coords, space_migratory_bdry_polygon, migr_bdry_contact_factor_mag):
    num_vertices = this_cell_coords.shape[0]
    are_nodes_in_migr_space = geometry.are_points_inside_polygon(this_cell_coords, space_migratory_bdry_polygon)
    
    result = np.zeros(num_vertices, dtype=np.float64)
...
def calculate_local_strains(this_cell_coords, length_edge_resting):
    ...
    num_vertices = this_cell_coords.shape[0]
    local_strains = np.empty(num_vertices, dtype=np.float64)
...
# -----------------------------------------------------------------
@nb.jit(nopython=True)  
def calculate_cytoplasmic_force(num_vertices, this_cell_coords, area_resting, stiffness_cytoplasmic, unit_inside_pointing_vectors):
    ...
    if current_area < area_resting:
        ...
        force_mag = area_strain*stiffness_cytoplasmic/num_vertices
...
def calculate_spring_edge_forces(num_vertices, this_cell_coords, stiffness_fn_a, stiffness_fn_b, stiffness_fn_c, length_edge_resting):
    ...
    edge_strains_plus = np.empty(num_vertices, dtype=np.float64)
    edge_strains_minus = np.empty(num_vertices, dtype=np.float64)
    ...
    return EFplus, EFminus

# -----------------------------------------------------------------
@nb.jit(nopython=True)
def determine_rac_rho_domination(rac_membrane_actives, rho_membrane_actives):
    num_vertices = rac_membrane_actives.shape[0]
    
    domination_array = np.empty(num_vertices, dtype=np.int64)
...
@nb.jit(nopython=True)  
def calculate_rgtpase_mediated_forces(num_vertices, this_cell_coords, rac_membrane_actives, rho_membrane_actives, threshold_force_rac_activity, threshold_force_rho_activity, max_force_rac, max_force_rho, unit_inside_pointing_vectors):   
    rgtpase_mediated_force_mags = np.zeros(num_vertices, dtype=np.float64)
    ...
    result = np.empty((num_vertices, 2), dtype=np.float64)
...
@nb.jit(nopython=True)
def calculate_external_forces(num_vertices, num_cells, this_ci, close_point_on_other_cells_to_each_node_exists, close_point_on_other_cells_to_each_node, close_point_on_other_cells_to_each_node_indices, close_point_on_other_cells_to_each_node_projection_factors, all_cells_centres, all_cells_node_forces, closeness_dist_criteria, unit_inside_pointing_vectors):
    F_external = np.zeros((num_vertices, 2), dtype=np.float64)
...
def calculate_forces(num_vertices, num_cells, this_ci, this_cell_coords, rac_membrane_actives, rho_membrane_actives, length_edge_resting, stiffness_fn_a, stiffness_fn_b, stiffness_fn_c, threhsold_force_rac_activity, threhsold_force_rho_activity, max_force_rac, max_force_rho, force_adh_constant, area_resting, stiffness_cytoplasmic, close_point_on_other_cells_to_each_node_exists, close_point_on_other_cells_to_each_node, close_point_on_other_cells_to_each_node_indices, close_point_on_other_cells_to_each_node_projection_factors, all_cells_centres, all_cells_node_forces, closeness_dist_criteria):
    ...
    rgtpase_mediated_forces = calculate_rgtpase_mediated_forces(num_vertices, this_cell_coords, rac_membrane_actives, rho_membrane_actives, threhsold_force_rac_activity, threhsold_force_rho_activity, max_force_rac, max_force_rho, unit_inside_pointing_vectors)
    ...
    #F_external = calculate_external_forces(num_vertices, num_cells, this_ci, close_point_on_other_cells_to_each_node_exists, close_point_on_other_cells_to_each_node, close_point_on_other_cells_to_each_node_indices, close_point_on_other_cells_to_each_node_projection_factors, all_cells_centres, all_cells_node_forces, closeness_dist_criteria, unit_inside_pointing_vectors)
    
    F = F_internal #+ F_external
    
    return F, EFplus, EFminus, rgtpase_mediated_forces, F_cytoplasmic, unit_inside_pointing_vectors
    
# =============================================================================
    
@nb.jit(nopython=True)
def calculate_global_strain(perimeter_resting, node_coords):
    current_perimeter = geometry.calculate_perimeter(node_coords)
    
    return (current_perimeter - perimeter_resting)/perimeter_resting

@nb.jit(nopython=True)
def calculate_average_tensile_local_strains(local_strains):
    sum_tensile_strains = 0.0
    
    for local_strain in local_strains:
        if local_strain > 0.0:
