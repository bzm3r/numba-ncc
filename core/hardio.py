...
def create_parameter_exploration_dataset(storefile_path, length_results_axis1):
    with h5py.File(storefile_path, "a") as f:
        f.create_dataset(
            "exploration_results",
            shape=(0, length_results_axis1),
            maxshape=(None, length_results_axis1),
            compression="gzip",
            compression_opts=9,
        )
...
def create_cell_dataset(cell_index, storefile_path, num_nodes, num_info_labels):
    with h5py.File(storefile_path, "a") as f:
        f.create_dataset(
            str(cell_index),
            shape=(0, num_nodes, num_info_labels),
            maxshape=(None, num_nodes, num_info_labels),
            compression="gzip",
            compression_opts=9,
        )
...
def append_parameter_exploration_data_to_dataset(
    new_last_executed_chunk_index, parameter_exploration_results, storefile_path
):
    with h5py.File(storefile_path, "a") as f:
...
def get_node_coords_until_tstep(cell_index, max_tstep, storefile_path):
    if max_tstep == None:
        ...
        node_coords = np.zeros(
            (x_coords.shape[0], x_coords.shape[1], 2), dtype=np.float64
        )
...
def get_cell_data_for_tsteps(cell_index, tsteps, data_labels, storefile_path):
    ...
    for data_label in data_labels:
        if type(tsteps) != type(np.array([])) and type(tsteps) != int:
            fetched_data.append(
                on_ram_dset[:, :, parameterorg.info_indices_dict[data_label]]
            )
        else:
            fetched_data.append(
                on_ram_dset[tsteps, :, parameterorg.info_indices_dict[data_label]]
            )
...
def get_data_until_timestep(cell_index, max_tstep, data_label, storefile_path):
    ...
    if on_ram_dset.shape[0] == 0:
        ...
        if max_tstep == None:
            ...
            return on_ram_dset[
                :max_tstep, :, parameterorg.info_indices_dict[data_label]
            ]


def get_multiple_data_until_timestep(
    cell_index, max_tstep, data_labels, data_types, storefile_path
):
    return_results = []
    ...
    for data_label, data_type in zip(data_labels, data_types):
        if data_type == "n":
            ...
            for i, basis_string in enumerate(["_x", "_y"]):
                if max_tstep == None:
                    data_for_this_basis = on_ram_dset[
                        :, :, parameterorg.info_indices_dict[data_label + basis_string]
                    ]
                else:
                    data_for_this_basis = on_ram_dset[
                        :max_tstep,
                ...
                if type(data) == type(None):
                    data = np.zeros(
                        (data_for_this_basis.shape[0], data_for_this_basis.shape[1], 2),
                        dtype=data_for_this_basis.dtype,
                    )
...
def check_if_data_label_is_vector_type(data_label):
    ...
    if data_label not in allowed_labels:
        raise Exception(
            "get_vector_data_* functions are not meant to be used with given label: {}. Allowed labels are: {}".format(
                data_label, allowed_labels
            )
        )
...
def get_vector_data_until_timestep(cell_index, max_tstep, data_label, storefile_path):
    ...
    data = None
    for i, basis_string in enumerate(["_x", "_y"]):
        data_for_this_basis = get_data_until_timestep(
            cell_index, max_tstep, data_label + basis_string, storefile_path
        )
