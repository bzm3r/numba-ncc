...
def verify_parameter_completeness(parameters_dict):
    if type(parameters_dict) != dict:
        raise Exception("parameters_dict is not a dict, instead: {}".type(parameters_dict))
...
#        print "*********************************"
        ...
        if parameters_dict['init_node_coords'].shape[0] != self.num_nodes:
            raise Exception("Number of node coords given for initialization does not match number of nodes! Given: {}, required: {}.".format(parameters_dict['init_node_coords'].shape[0], self.num_nodes))
        ...
        self.perimeter_resting = self.length_edge_resting*self.num_nodes
        edgeplus_lengths = geometry.calculate_edgeplus_lengths(self.curr_node_coords)   
        self.init_average_edge_lengths = np.average(geometry.calculate_average_edge_length_around_nodes(edgeplus_lengths))
        ...
        # K L = F L^-1 = M T^-2
        self.stiffness_edge_resting = (parameters_dict['stiffness_edge_resting'])/(self.ML_T2/self.L)
        self.stiffness_edge_high_tensile_strain = (parameters_dict['stiffness_edge_high_tensile_strain'])/(self.ML_T2/self.L)
        self.high_tensile_strain_definition = parameters_dict['high_tensile_strain_definition']
        
        self.stiffness_edge_high_compressive_strain = 10*self.stiffness_edge_resting
        
        self.stiffness_fn_a, self.stiffness_fn_b, self.stiffness_fn_c = mechanics.calculate_exponential_stiffness_function_parameters([-1*self.high_tensile_strain_definition, 0.0, self.high_tensile_strain_definition], [self.stiffness_edge_high_compressive_strain, self.stiffness_edge_resting, self.stiffness_edge_high_tensile_strain])
        ...
        self.closeness_dist_squared_criteria = parameters_dict['closeness_dist_squared_criteria']/(self.L**2)
        self.closeness_dist_squared_criteria_0_until = self.closeness_dist_squared_criteria*9
        ...
        self.threshold_rac_autoact = parameters_dict['threshold_rac_activity']/(self.C_total*self.num_nodes*self.init_average_edge_lengths)
        self.exponent_rac_autoact = parameters_dict['hill_exponent']
        
        self.kdgtp_rac_baseline = parameters_dict['kdgtp_rac_baseline']*self.T
        self.kdgtp_rho_mediated_rac_inhib_baseline = parameters_dict['kdgtp_rho_mediated_rac_inhib_baseline']*self.T
        
        self.threshold_rho_mediated_rac_inhib = parameters_dict['threshold_rho_activity']/(self.H_total*self.num_nodes*self.init_average_edge_lengths)
        ...
        self.tension_mediated_rac_inhibition_magnitude = parameters_dict['tension_mediated_rac_inhibition_magnitude']
        ...
        self.threshold_rho_autoact = parameters_dict['threshold_rho_activity']/(self.H_total*self.num_nodes*self.init_average_edge_lengths)
        self.exponent_rho_autoact = parameters_dict['hill_exponent']
        
        self.kdgtp_rho_baseline = parameters_dict['kdgtp_rho_baseline']*self.T
        self.kdgtp_rac_mediated_rho_inhib_baseline = parameters_dict['kdgtp_rac_mediated_rho_inhib_baseline']*self.T
        
        self.threshold_rac_mediated_rho_inhib = parameters_dict['threshold_rac_activity']/(self.C_total*self.num_nodes*self.init_average_edge_lengths)
        ...
        given_space_physical_bdry_polygon = parameters_dict['space_physical_bdry_polygon']
        if given_space_physical_bdry_polygon.shape[0] > 0:
            self.exists_space_physical_bdry_polygon = 1
            self.space_physical_bdry_polygon = given_space_physical_bdry_polygon/self.L
            
        self.space_migratory_bdry_polygon = parameters_dict['space_migratory_bdry_polygon']/self.L
        
        self.interaction_factor_migr_bdry_contact = parameters_dict['interaction_factor_migr_bdry_contact']
        self.interaction_factors_intercellular_contact_per_celltype = parameters_dict['interaction_factors_intercellular_contact_per_celltype']
        
        #==============================================================
        self.interaction_factors_coa_per_celltype = parameters_dict['interaction_factors_coa_per_celltype']
        ...
        self.all_cellwide_phase_var_indices = [parameterorg.rac_cytosolic_gdi_bound_index, parameterorg.rho_cytosolic_gdi_bound_index, parameterorg.global_strain_index]
        ...
        self.nodal_pars_indices = [parameterorg.kgtp_rac_index, parameterorg.kgtp_rho_index, parameterorg.kdgtp_rac_index, parameterorg.kdgtp_rho_index, parameterorg.kdgdi_rac_index, parameterorg.kdgdi_rho_index, parameterorg.interaction_factors_intercellular_contact_per_celltype_index, parameterorg.migr_bdry_contact_index]
...
# -----------------------------------------------------------------
    def insert_state_array_into_system_history(self, state_array, tstep):
        ...
        self.system_history[access_index, :, self.nodal_phase_var_indices] = nodal_phase_vars
...
# -----------------------------------------------------------------
    def initialize_cell(self, init_node_coords, biased_rgtpase_distrib_defn, init_rgtpase_cytosol_frac, init_rgtpase_membrane_inactive_frac, init_rgtpase_membrane_active_frac, init_rho_gtpase_conditions):
        ...
        chemoattractant_gradient_on_nodes = np.zeros(self.num_nodes, dtype=np.float64)
        self.system_history[access_index, :, parameterorg.chemoattractant_gradient_on_nodes_index] = np.zeros(self.num_nodes, dtype=np.float64)
        ...
        close_point_smoothness_factors = np.zeros((self.num_nodes, self.num_cells_in_environment), dtype=np.float64)
        ...
        F, EFplus, EFminus, F_rgtpase, F_cytoplasmic, unit_inside_pointing_vectors  = mechanics.calculate_forces(self.num_nodes, self.num_cells_in_environment, self.cell_index, node_coords, rac_membrane_actives, rho_membrane_actives, self.length_edge_resting, self.stiffness_fn_a, self.stiffness_fn_b, self.stiffness_fn_c, self.threshold_force_rac_activity, self.threshold_force_rho_activity, self.max_force_rac, self.max_force_rho, self.force_adh_constant, self.area_resting, self.stiffness_cytoplasmic, close_point_on_other_cells_to_each_node_exists, close_point_on_other_cells_to_each_node, close_point_on_other_cells_to_each_node_indices, close_point_on_other_cells_to_each_node_projection_factors, all_cells_centres, all_cells_node_forces, self.closeness_dist_criteria)
        
        global_strain = mechanics.calculate_global_strain(self.perimeter_resting, init_node_coords)

        self.system_history[access_index, 0, parameterorg.global_strain_index] = global_strain
        ...
        self.system_history[access_index, :, parameterorg.kgtp_rac_index] = chemistry.calculate_kgtp_rac(conc_rac_membrane_actives, migr_bdry_contact_factors, self.exponent_rac_autoact, self.threshold_rac_autoact, self.kgtp_rac_baseline, self.kgtp_rac_autoact_baseline, coa_signals, chemoattractant_gradient_on_nodes, self.randomization_rac_kgtp_multipliers, intercellular_contact_factors, close_point_smoothness_factors)
        
        self.system_history[access_index, :, parameterorg.kgtp_rho_index] = chemistry.calculate_kgtp_rho(conc_rho_membrane_actives, intercellular_contact_factors, migr_bdry_contact_factors, self.exponent_rho_autoact, self.threshold_rho_autoact, self.kgtp_rho_baseline, self.kgtp_rho_autoact_baseline)
        
        global_strain = mechanics.calculate_global_strain(self.perimeter_resting, node_coords)
        
        self.system_history[access_index, :, parameterorg.kdgtp_rac_index] = chemistry.calculate_kdgtp_rac(conc_rho_membrane_actives, self.exponent_rho_mediated_rac_inhib, self.threshold_rho_mediated_rac_inhib, self.kdgtp_rac_baseline, self.kdgtp_rho_mediated_rac_inhib_baseline, intercellular_contact_factors, migr_bdry_contact_factors, self.tension_mediated_rac_inhibition_half_strain, self.tension_mediated_rac_inhibition_magnitude, global_strain)
        
        self.system_history[access_index, :, parameterorg.kdgtp_rho_index] = chemistry.calculate_kdgtp_rho(conc_rac_membrane_actives, self.exponent_rac_mediated_rho_inhib, self.threshold_rac_mediated_rho_inhib, self.kdgtp_rho_baseline, self.kdgtp_rac_mediated_rho_inhib_baseline)
        
        # update mechanics parameters
        self.system_history[access_index, :, [parameterorg.F_x_index, parameterorg.F_y_index]] = np.transpose(F)
        self.system_history[access_index, :, [parameterorg.EFplus_x_index, parameterorg.EFplus_y_index]] = np.transpose(EFplus)
                ...
                if rgtpase_label in label:
                    if '_membrane_' in label:
                        ...
                        if distrib_type == "unbiased random":
                            ...
                            raise Exception("Invalid initial rGTPase distribution type provided! ({})".format(distrib_type))
                    ...
                    self.system_history[access_index, :, eval("parameterorg." + label + "_index")] = rgtpase_distrib
...
# -----------------------------------------------------------------
    def set_next_state(self, next_state_array, this_cell_index, num_cells, intercellular_squared_dist_array, line_segment_intersection_matrix, all_cells_node_coords, all_cells_node_forces, are_nodes_inside_other_cells, chemoattractant_gradient_on_nodes, close_point_on_other_cells_to_each_node_exists, close_point_on_other_cells_to_each_node, close_point_on_other_cells_to_each_node_indices, close_point_on_other_cells_to_each_node_projection_factors, close_point_smoothness_factors):
        ...
        self.system_history[next_tstep_system_history_access_index, :, parameterorg.edge_lengths_index] = edge_lengths
        ...
        if self.space_migratory_bdry_polygon.size == 0:
            ...
            migr_bdry_contact_factors = mechanics.calculate_migr_bdry_contact_factors(node_coords, self.space_migratory_bdry_polygon, self.interaction_factor_migr_bdry_contact)
        ...
        if self.verbose == True:
            ...
            print("max_ext: ", np.max(chemoattractant_gradient_on_nodes))
            print("min_ext: ", np.min(chemoattractant_gradient_on_nodes))
        ...
        self.system_history[next_tstep_system_history_access_index, :, parameterorg.chemoattractant_gradient_on_nodes_index] = chemoattractant_gradient_on_nodes
        ...
        rho_cytosolic_gdi_bound = 1 - np.sum(rho_membrane_actives) - np.sum(self.system_history[next_tstep_system_history_access_index, :, parameterorg.rho_membrane_inactive_index])
        
        insertion_array = np.zeros(self.num_nodes)
        insertion_array[0] = 1
        
        self.system_history[next_tstep_system_history_access_index, :, parameterorg.rac_cytosolic_gdi_bound_index] = rac_cytosolic_gdi_bound*insertion_array
        self.system_history[next_tstep_system_history_access_index, :, parameterorg.rho_cytosolic_gdi_bound_index] = rho_cytosolic_gdi_bound*insertion_array
...
#        print "stiffness_cytoplasmic: ", self.stiffness_cytoplasmic
        ...
        F, EFplus, EFminus, F_rgtpase, F_cytoplasmic, unit_inside_pointing_vecs = mechanics.calculate_forces(self.num_nodes, self.num_cells_in_environment, self.cell_index, node_coords, rac_membrane_actives, rho_membrane_actives, self.length_edge_resting, self.stiffness_fn_a, self.stiffness_fn_b, self.stiffness_fn_c, self.threshold_force_rac_activity, self.threshold_force_rho_activity, self.max_force_rac, self.max_force_rho, self.force_adh_constant, self.area_resting, self.stiffness_cytoplasmic, close_point_on_other_cells_to_each_node_exists, close_point_on_other_cells_to_each_node, close_point_on_other_cells_to_each_node_indices, close_point_on_other_cells_to_each_node_projection_factors, all_cells_centres, all_cells_node_forces, self.closeness_dist_criteria)
...
#        print "F_cytoplasmic: ", np.min(printing_fcyto), np.max(printing_fcyto) 
        ...
        kgtp_rac_per_node = chemistry.calculate_kgtp_rac(conc_rac_membrane_actives, migr_bdry_contact_factors, self.exponent_rac_autoact, self.threshold_rac_autoact, self.kgtp_rac_baseline, self.kgtp_rac_autoact_baseline, coa_signals, chemoattractant_gradient_on_nodes, self.randomization_rac_kgtp_multipliers, intercellular_contact_factors, close_point_smoothness_factors)
        
        kgtp_rho_per_node = chemistry.calculate_kgtp_rho(conc_rho_membrane_actives, intercellular_contact_factors, migr_bdry_contact_factors, self.exponent_rho_autoact, self.threshold_rho_autoact, self.kgtp_rho_baseline, self.kgtp_rho_autoact_baseline)
        ...
        self.system_history[next_tstep_system_history_access_index, :, parameterorg.kdgtp_rac_index] = chemistry.calculate_kdgtp_rac(conc_rho_membrane_actives, self.exponent_rho_mediated_rac_inhib, self.threshold_rho_mediated_rac_inhib, self.kdgtp_rac_baseline, self.kdgtp_rho_mediated_rac_inhib_baseline, intercellular_contact_factors, migr_bdry_contact_factors, self.tension_mediated_rac_inhibition_half_strain, self.tension_mediated_rac_inhibition_magnitude, global_strain)

        self.system_history[next_tstep_system_history_access_index, :, parameterorg.kdgtp_rho_index] = chemistry.calculate_kdgtp_rho(conc_rac_membrane_actives, self.exponent_rac_mediated_rho_inhib, self.threshold_rac_mediated_rho_inhib, self.kdgtp_rho_baseline, self.kdgtp_rac_mediated_rho_inhib_baseline)
        
        # update mechanics parameters
        self.system_history[next_tstep_system_history_access_index, :, [parameterorg.F_x_index, parameterorg.F_y_index]] = np.transpose(F)
        self.system_history[next_tstep_system_history_access_index, :, [parameterorg.EFplus_x_index, parameterorg.EFplus_y_index]] = np.transpose(EFplus)
...
# -----------------------------------------------------------------
    def trim_system_history(self, environment_tpoint):
        ...
        trimmed_system_history[0] = self.system_history[self.get_system_history_access_index(self.curr_tpoint)]
...
# -----------------------------------------------------------------
    def execute_step(self, this_cell_index, num_nodes, all_cells_node_coords, all_cells_node_forces, intercellular_squared_dist_array, line_segment_intersection_matrix, chemoattractant_gradient_fn, be_talkative=False):
        ...
        if self.skip_dynamics == False:            
            ...
            chemoattractant_gradient_on_nodes = [chemoattractant_gradient_fn(x*self.L/1e-6) for x in all_cells_node_coords[this_cell_index]]
