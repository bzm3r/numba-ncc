...
# -----------------------------------------------------------------
    ...
    def calculate_cell_bounding_boxes(self, num_cells, init_cell_radius, cell_group_bounding_box, cell_placement_method=""):
        ...
        if total_cell_group_area > cell_group_bounding_box_area:
            raise Exception("Cell group bounding box is not big enough to contain all cells given init_cell_radius constraint.")
    ...
    def execute_system_dynamics_in_random_sequence(self, t, cells_node_distance_matrix, cells_bounding_box_array, cells_line_segment_intersection_matrix, environment_cells_node_coords, environment_cells_node_forces, environment_cells, centroid_drifts, recalc_geometry):        
        ...
        for cell_index in execution_sequence:
            ...
            if current_cell.skip_dynamics == False:
                ...
                if recalc_geometry[cell_index]:
                    cells_node_distance_matrix, cells_line_segment_intersection_matrix =  geometry.update_line_segment_intersection_and_dist_squared_matrices_old(cell_index, self.num_cells, self.num_nodes, environment_cells_node_coords, cells_bounding_box_array, self.space_migratory_bdry_polygon, self.space_physical_bdry_polygon, cells_node_distance_matrix, cells_line_segment_intersection_matrix)
                    #cells_node_distance_matrix, cells_line_segment_intersection_matrix = geometry.update_line_segment_intersection_and_dist_squared_matrices(4, self.geometry_tasks_per_cell[cell_index], self.num_cells, self.num_nodes, environment_cells_node_coords, cells_bounding_box_array, cells_node_distance_matrix, cells_line_segment_intersection_matrix, self.space_migratory_bdry_polygon, self.space_physical_bdry_polygon)
                else:
                    #cells_node_distance_matrix = geometry.update_distance_squared_matrix(4, self.geometry_tasks_per_cell[cell_index], self.num_cells, self.num_nodes, environment_cells_node_coords, cells_node_distance_matrix)
...
#                
                ...
                tension_mediated_rac_inhibition_half_strain = this_cell.tension_mediated_rac_inhibition_half_strain
...
# ----------------------------------------------------------------- 
    ...
    def execute_system_dynamics(self, animation_settings,  produce_intermediate_visuals=True, produce_final_visuals=True, elapsed_timesteps_before_producing_intermediate_graphs=2500, elapsed_timesteps_before_producing_intermediate_animations=5000, given_pool_for_making_visuals=None):
        ...
        if self.mode == MODE_EXECUTE and self.simulation_execution_enabled:
            ...
            cells_node_distance_matrix, cells_line_segment_intersection_matrix = geometry.create_initial_line_segment_intersection_and_dist_squared_matrices_old(num_cells, num_nodes, cells_bounding_box_array, environment_cells_node_coords, self.space_migratory_bdry_polygon, self.space_physical_bdry_polygon)
