...
def place_init_cell_randomly(cell_centers, cell_diameter, corridor_origin, corridor_height, box_height, box_width,
...
def update_theta_bin_probabilities(target_bin_index, theta_bin_probabilities):
    ...
    if np.abs(1.0 - np.sum(theta_bin_probabilities)) > 1e-6:
        raise Exception(
            "theta_bin_probabilities: {}\nsum: {}\ndelta_p: {}\ninterestin_bins: {}".format(theta_bin_probabilities,
                                                                                            np.sum(
                                                                                                theta_bin_probabilities),
                                                                                            delta_p, interesting_bins))
...
def try_placing_cell_randomly(last_successful_anchor_index, last_placed_cell_index, cell_centers, theta_bins,
    ...
    for ti in range(num_trials):
        ...
        if is_collision(last_placed_cell_index, cell_centers, cell_diameter, corridor_origin, corridor_height, test_x,
                        test_y):
            theta_bin_probabilities = update_theta_bin_probabilities(find_relevant_bin_index(theta, theta_bins),
                                                                     theta_bin_probabilities)
        else:
...
def place_cells_randomly(num_cells, cell_diameter, corridor_origin, corridor_height, box_width, box_height,
    ...
    while num_cells_placed != num_cells:
        cell_index, cell_centers = try_placing_cell_randomly(possible_anchor_indices[trial_anchor_index],
                                                             num_cells_placed - 1, cell_centers, theta_bins,
                                                             cell_diameter, corridor_origin, corridor_height,
                                                             box_height, box_width, max_placement_distance_factor)
...
class Environment():
    ...
    def __init__(self, environment_name='', num_timesteps=0, space_physical_bdry_polygon=np.array([], dtype=np.float64),
                 ...
                 max_placement_distance_factor=1.0, init_random_cell_placement_x_factor=0.25, convergence_test=False,
                 shell_environment=False, graph_group_centroid_splits=False):
        ...
        if not shell_environment:
            ...
            for n in num_nodes_per_cell[1:]:
                if n != self.num_nodes:
                    raise Exception("There exists a cell with number of nodes different from other cells!")
        else:
        ...
        for cell_index in range(self.num_cells):
            if self.environment_dir != None:
                hardio.create_cell_dataset(cell_index, self.storefile_path, self.num_nodes,
                                           parameterorg.num_info_labels)

        self.all_geometry_tasks = np.array(
            geometry.create_dist_and_line_segment_interesection_test_args(self.num_cells, self.num_nodes),
            dtype=np.int64)
        self.geometry_tasks_per_cell = np.array([
                                                    geometry.create_dist_and_line_segment_interesection_test_args_relative_to_specific_cell(
                                                        ci, self.num_cells, self.num_nodes) for ci in
                                                    range(self.num_cells)], dtype=np.int64)
    ...
    def load_from_pickle(self, pickle_fp):
        ...
        if env == None:
            raise Exception("Couldn't open pickled environment at: {}".format(pickle_fp))
        else:
    ...
    def make_cells(self):
        ...
        for cell_group_index, cell_group_defn in enumerate(self.cell_group_defns):
            cells_in_group, init_cell_bounding_boxes = self.create_cell_group(self.num_timesteps, cell_group_defn,
                                                                              cell_group_index, cell_index_offset)
    ...
    def create_cell_group(self, num_timesteps, cell_group_defn, cell_group_index, cell_index_offset):
        ...
        init_cell_bounding_boxes = self.calculate_cell_bounding_boxes(num_cells, init_cell_radius,
                                                                      cell_group_bounding_box,
                                                                      cell_placement_method=self.cell_placement_method)
        ...
        for cell_number, bounding_box in enumerate(init_cell_bounding_boxes):
            ...
            cell_parameter_dict.update(
                [('biased_rgtpase_distrib_defn', bias_defn), ('init_node_coords', init_node_coords),
                 ('length_edge_resting', length_edge_resting), ('area_resting', area_resting)])
            ...
            if len(undefined_labels) > 0:
                raise Exception("The following labels are not yet defined: {}".format(undefined_labels))

            if self.parameter_explorer_run and self.parameter_explorer_init_rho_gtpase_conditions != None:
                new_cell = cell.Cell(str(cell_group_name) + '_' + str(cell_index), cell_group_index, cell_index,
                                     integration_params, num_timesteps, self.T, self.num_cells,
                                     self.max_timepoints_on_ram, self.verbose, cell_parameter_dict,
    ...
    def calculate_cell_bounding_boxes(self, num_cells, init_cell_radius, cell_group_bounding_box,
        ...
        if cell_placement_method == "":
            ...
            for ci in range(num_cells):
                cell_bounding_boxes[ci] = [cell_x_coords[xi], cell_x_coords[xi] + x_step, cell_y_coords[yi],
            ...
            cell_bounding_boxes = generate_bounding_boxes_from_centers(0.5*cell_diameter,
                                                                       place_cells_randomly(self.num_cells,
                                                                                            ...
                                                                                            self.init_random_cell_placement_x_factor,
                                                                                            self.max_placement_distance_factor))
    ...
    def create_default_init_cell_node_coords(self, bounding_box, init_cell_radius, num_nodes):
        ...
        cell_node_coords = np.array([[x + cell_centre[0], y + cell_centre[1]] for x, y in cell_node_coords],
                                    dtype=np.float64)
    ...
    # -----------------------------------------------------------------
    def execute_system_dynamics_in_random_sequence(self, t, cells_node_distance_matrix, cells_bounding_box_array,
                                                   cells_line_segment_intersection_matrix,
                                                   environment_cells_node_coords, environment_cells_node_forces,
                                                   environment_cells, centroid_drifts, recalc_geometry):
        ...
        for cell_index in execution_sequence:
            ...
            if current_cell.skip_dynamics == False:
                ...
                if recalc_geometry[cell_index]:
                    ...
                    geometry.update_line_segment_intersection_and_dist_squared_matrices(4, self.geometry_tasks_per_cell[
                        cell_index], self.num_cells, self.num_nodes, environment_cells_node_coords,
                    ...
                    geometry.update_distance_squared_matrix(4, self.geometry_tasks_per_cell[cell_index], self.num_cells,
                                                            self.num_nodes, environment_cells_node_coords,
                                                            cells_node_distance_matrix)
    ...
    def do_data_analysis_and_make_visuals(self, t, save_dir, animation_settings, animation_obj, produce_graphs,
                                          produce_animation, num_polar_graph_bins=10):
        if self.environment_dir != None:
            data_dict = {}
        else:
        ...
        if produce_graphs:
            ...
            for cell_index in range(self.num_cells):
                ...
                averaged_score, scores_per_tstep = cu.calculate_rgtpase_polarity_score(cell_index, self.storefile_path,
                ...
                data_dict = datavis.graph_important_cell_variables_over_time(self.T/60.0, cell_Ls[cell_index],
                                                                             ...
                                                                             save_name='C={}'.format(
                                                                                 cell_index) + '_important_cell_vars_graph_T={}'.format(
                                                                                 t - 1), save_dir=save_dir_for_cell,
                                                                             max_tstep=t,
            ...
            protrusion_lifetime_and_direction_data_per_cell = cu.collate_protrusion_data(self.num_cells, self.T,
                                                                                         self.storefile_path,
                                                                                         max_tstep=t)
            datavis.add_to_general_data_structure(data_dict, [
                ("all_cell_protrusion_lifetimes_and_directions", protrusion_lifetime_and_direction_data_per_cell)])
    ...
    def get_empty_cells(self):
        return [self.get_empty_cell(ci) for ci in range(self.num_cells)]
    ...
    def dump_to_store(self, tpoint):
        ...
        for cell_index in range(self.num_cells):
            ...
            if this_cell.last_trim_timestep < 0:
                hardio.append_cell_data_to_dataset(cell_index, this_cell.system_history[:access_index + 1],
                                                   self.storefile_path)
                this_cell.trim_system_history(tpoint)
            elif this_cell.last_trim_timestep < tpoint:
                hardio.append_cell_data_to_dataset(cell_index, this_cell.system_history[1:access_index + 1],
                                                   self.storefile_path)
                this_cell.trim_system_history(tpoint)
            else:
                continue
    ...
    def init_from_store(self, environment_wide_variable_defns, tpoint=None, simulation_execution_enabled=True):
        ...
        new_num_timesteps = environment_wide_variable_defns['num_timesteps']

        if old_num_timesteps > new_num_timesteps:
            self.num_timesteps = old_num_timesteps

        if old_num_timesteps < new_num_timesteps:
            self.extend_simulation_runtime(new_num_timesteps)
    ...
    def execute_system_dynamics(self, animation_settings, produce_intermediate_visuals=True, produce_graphs=True,
                                produce_animation=True, elapsed_timesteps_before_producing_intermediate_graphs=2500,
        ...
        if self.mode == MODE_EXECUTE and self.simulation_execution_enabled:
            ...
            cells_bounding_box_array = geometry.create_initial_bounding_box_polygon_array(num_cells, num_nodes,
            ...
            cells_node_distance_matrix, cells_line_segment_intersection_matrix = geometry.create_initial_line_segment_intersection_and_dist_squared_matrices_old(
                num_cells, num_nodes, cells_bounding_box_array, environment_cells_node_coords,
                self.space_migratory_bdry_polygon, self.space_physical_bdry_polygon)
            ...
            if self.environment_dir == None:
                ...
                if produce_animation:
                    animation_obj = animator.EnvironmentAnimation(self.environment_dir, self.environment_name,
                                                                  self.num_cells, self.num_nodes, self.num_timepoints,
                                                                  cell_group_indices, cell_Ls, cell_etas,
                                                                  cell_skip_dynamics, self.storefile_path,
                                                                  **animation_settings)
                else:
                    animation_obj = None
                ...
                for t in self.timepoints[self.curr_tpoint:-1]:
                    ...
                    if type(produce_intermediate_visuals) == np.ndarray:
                        if t != 0 and t in produce_intermediate_visuals:
                            ...
                            self.do_data_analysis_and_make_visuals(t, data_save_dir, animation_settings, animation_obj,
                                                                   True, True)
                    ...
                    cells_node_distance_matrix, cells_bounding_box_array, cells_line_segment_intersection_matrix, environment_cells_node_coords, environment_cells_node_forces = self.execute_system_dynamics_in_random_sequence(
                        t, cells_node_distance_matrix, cells_bounding_box_array, cells_line_segment_intersection_matrix,
                        environment_cells_node_coords, environment_cells_node_forces, environment_cells,
                        centroid_drifts, recalc_geometry)
