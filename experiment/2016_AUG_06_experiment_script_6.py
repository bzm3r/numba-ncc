...
biased_rgtpase_distrib_defn_dicts += [[{'default': ['biased random', np.array([-0.125*np.pi, 0.125*np.pi]), 0.3]}, {'default': ['biased random', np.array([-0.125*np.pi, 0.125*np.pi]) + np.pi, 0.3]}]]
...
for si in xrange(num_sub_experiments):
    ...
    environment_name_format_strings.append( eu.form_base_environment_name_format_string(EXPERIMENT_NUMBER, np.sum(NU+M_CELLS_IN_BOXES), str(np.round(TOTAL_TIME/(3600.), decimals=2))+"h", NUM_TIMESTEPS, NUM_NODES, HEIGHT_MIGR_CORRIDOR, WIDTH_MIGR_CORRIDOR))
