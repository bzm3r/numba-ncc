...
experiment_dir = eu.get_experiment_directory_path(BASE_OUTPUT_DIR, DATE_STR, EXPERIMENT_NUMBER)
...
plate_width, plate_height = 1.2*9*CELL_DIAMETER, 2*(box_heights[0] + physical_bdry_polygon_extra*2 + 10)#1500, (box_heights[0] + physical_bdry_polygon_extra*2)*2

x_space_between_boxes = [3*CELL_DIAMETER, 2*CELL_DIAMETER, 2*CELL_DIAMETER]

x_offset, y_offset = 10 + 2*CELL_DIAMETER, 10#0.5*(plate_width - 0.5*box_widths[1]), 0.5*(plate_height - 0.5*box_heights[1])
...
space_migratory_bdry_polygon, space_physical_bdry_polygon = eu.make_space_polygons(make_migr_poly, make_phys_poly, WIDTH_MIGR_CORRIDOR, HEIGHT_MIGR_CORRIDOR, 10, 10, physical_bdry_polygon_extra=physical_bdry_polygon_extra)
...
intercellular_contact_factor_magnitudes_defn_dicts_per_sub_experiment += [{0: {0: self_cil, 1: self_cil}, 1: {0: self_cil, 1: self_cil}}]
biased_rgtpase_distrib_defn_dicts += [[{'default': ['biased random', np.array([-0.25*np.pi, 0.25*np.pi]), 0.3]}, {'default': ['biased random', np.array([-0.25*np.pi, 0.25*np.pi]) + np.pi, 0.3]}]]
external_gradient_fn_per_subexperiment += [lambda x: 0.0]
parameter_override_dicts_per_sub_experiment += [[eu.update_pd_with_keyvalue_tuples(base_parameter_dict, [('halfmax_coa_sensing_dist_multiplier', 4.4, 4.4), ('kdgdi_rac_estimate_multiplier', 0.2, 1.0*0.2), ('kdgdi_rho_estimate_multiplier', 0.2, 1.0*0.2), ('kgdi_rac_estimate_multiplier', 1, 1.0), ('kgdi_rho_estimate_multiplier', 1, 1), ('kdgtp_rac_mediated_rho_inhib_multiplier', 500, 800), ('kdgtp_rac_multiplier', 20, 20*(2/3.)), ('kdgtp_rho_mediated_rac_inhib_multiplier', 3000, 500), ('kdgtp_rho_multiplier', 20, 20*(2/3.)), ('kgtp_rac_autoact_multiplier', 250, 1.1*200*2.5), ('kgtp_rac_multiplier', 20, 20), ('kgtp_rho_autoact_multiplier', 125, 105*1.0), ('kgtp_rho_multiplier', 20, 20), ('max_protrusive_node_velocity', 1e-06, 0.75*0.25e-6), ('randomization', False, True), ('randomization_scheme', 'wipeout', 'kgtp_rac_multipliers'), ('randomization_time_mean', 20, 20), ('randomization_time_variance_factor', 0.25, 0.01), ('randomization_magnitude', 1.0, 10), ('sigma_rac', 2e-05, 3.2e-05), ('sigma_rho_multiplier', 0.2, 0.2), ('force_adh_constant', 1.0, 0.5), ('force_rac_exp', 3, 3), ('force_rho_exp', 3, 3), ('force_rac_threshold_multiplier', 0.5, 0.9*1.75), ('force_rho_threshold_multiplier', 0.5, 0.9*1.75), ('skip_dynamics', False, False), ('stiffness_edge', 5e-10, 0.75*3.8e-10), ('tension_fn_type', 0, 5), ('tension_mediated_rac_inhibition_half_strain', 0.05, 0.05*0.6), ('threshold_rac_autoact_multiplier', 0.5, 0.5), ('threshold_rac_mediated_rho_inhib_multiplier', 0.5, 0.5), ('threshold_rho_autoact_multiplier', 0.5, 0.3), ('threshold_rho_mediated_rac_inhib_multiplier', 0.5, 0.5), ('coa_sensitivity_percent_drop_over_cell_diameter', 0.25, -1), ('coa_belt_offset_multiplier', 1.5, 1.5)])]*2]
...
biased_rgtpase_distrib_defn_dicts += [[{'default': ['biased random', np.array([-0.25*np.pi, 0.25*np.pi]), 0.3]}, {'default': ['biased random', np.array([-0.25*np.pi, 0.25*np.pi]) + np.pi, 0.3]}]]
external_gradient_fn_per_subexperiment += [lambda x: 0.0]
parameter_override_dicts_per_sub_experiment += [[eu.update_pd_with_keyvalue_tuples(base_parameter_dict, [('halfmax_coa_sensing_dist_multiplier', 4.4, 4.4), ('kdgdi_rac_estimate_multiplier', 0.2, 1.0*0.2), ('kdgdi_rho_estimate_multiplier', 0.2, 1.0*0.2), ('kgdi_rac_estimate_multiplier', 1, 1.0), ('kgdi_rho_estimate_multiplier', 1, 1), ('kdgtp_rac_mediated_rho_inhib_multiplier', 500, 800), ('kdgtp_rac_multiplier', 20, 20*(2/3.)), ('kdgtp_rho_mediated_rac_inhib_multiplier', 3000, 500), ('kdgtp_rho_multiplier', 20, 20*(2/3.)), ('kgtp_rac_autoact_multiplier', 250, 1.1*200*2.5), ('kgtp_rac_multiplier', 20, 20), ('kgtp_rho_autoact_multiplier', 125, 105*1.0), ('kgtp_rho_multiplier', 20, 20), ('max_protrusive_node_velocity', 1e-06, 0.75*0.25e-6), ('randomization', False, False), ('randomization_scheme', 'wipeout', 'kgtp_rac_multipliers'), ('randomization_time_mean', 20, 20), ('randomization_time_variance_factor', 0.25, 0.01), ('randomization_magnitude', 1.0, 10), ('sigma_rac', 2e-05, 3.2e-05), ('sigma_rho_multiplier', 0.2, 0.2), ('force_adh_constant', 1.0, 0.5), ('force_rac_exp', 3, 3), ('force_rho_exp', 3, 3), ('force_rac_threshold_multiplier', 0.5, 0.9*1.75), ('force_rho_threshold_multiplier', 0.5, 0.9*1.75), ('skip_dynamics', False, False), ('stiffness_edge', 5e-10, 0.75*3.8e-10), ('tension_fn_type', 0, 5), ('tension_mediated_rac_inhibition_half_strain', 0.05, 0.05*0.6), ('threshold_rac_autoact_multiplier', 0.5, 0.5), ('threshold_rac_mediated_rho_inhib_multiplier', 0.5, 0.5), ('threshold_rho_autoact_multiplier', 0.5, 0.3), ('threshold_rho_mediated_rac_inhib_multiplier', 0.5, 0.5), ('coa_sensitivity_percent_drop_over_cell_diameter', 0.25, -1), ('coa_belt_offset_multiplier', 1.5, 1.5)])]*2]
...
for si in xrange(num_sub_experiments):
    ...
    environment_name_format_strings.append( eu.form_base_environment_name_format_string(EXPERIMENT_NUMBER, np.sum(NUM_CELLS_IN_BOXES), str(np.round(TOTAL_TIME/(3600.), decimals=2))+"h", NUM_TIMESTEPS, NUM_NODES, HEIGHT_MIGR_CORRIDOR, WIDTH_MIGR_CORRIDOR))
...
if __name__ == '__main__':
    ...
    if RUN_EXPERIMENTS == True:
        ...
        eu.run_experiments(experiment_dir, environment_name_format_strings, environment_wide_variable_defns, user_cell_group_defns_per_subexperiment, experiment_descriptions_per_subexperiment, external_gradient_fn_per_subexperiment, num_experiment_repeats=NUM_EXPERIMENT_REPEATS, animation_settings=animation_settings, produce_intermediate_visuals=np.array([]), produce_final_visuals=True, full_print=True, delete_and_rerun_experiments_without_stored_env=True)
    ...
    if RUN_ANALYSIS == True:
        ...
        for si in xrange(len(parameter_override_dicts_per_sub_experiment)):
            ...
            for rpt_number in xrange(NUM_EXPERIMENT_REPEATS):
                ...
                environment_dir = eu.get_environment_directory_path(experiment_dir, environment_name)
