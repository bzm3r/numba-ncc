...
EXPERIMENT_NUMBER = 1
experiment_description = "small group"

BASE_OUTPUT_DIR = "A:\\numba-ncc\\output\\"
DATE_STR = "2016_MAR_19"

experiment_dir = get_experiment_directory_path(
    BASE_OUTPUT_DIR, DATE_STR, EXPERIMENT_NUMBER
)

TOTAL_TIME = 5 * 60 * 60  # 32000#(60*60)*3
TIMESTEP_LENGTH = 1.0 / 0.5
...
box_y_offsets = [
    y_offset,
    y_offset - 2 * CELL_DIAMETER,
    y_offset + 2 * CELL_DIAMETER,
    y_offset + 1.5 * CELL_DIAMETER,
]

WIDTH_MIGR_CORRIDOR = 5 * box_widths[0]
HEIGHT_MIGR_CORRIDOR = 3 * box_heights[0]

make_migr_poly = True
make_phys_poly = False

space_migratory_bdry_polygon, space_physical_bdry_polygon = make_space_polygons(
    make_migr_poly, make_phys_poly, WIDTH_MIGR_CORRIDOR, HEIGHT_MIGR_CORRIDOR, 10, 10
)
    ...
    [
        {
            "default": [
                "unbiased random",
                np.array([-np.pi / 4, np.pi / 4]) + np.pi / 32 + np.pi,
                0.1,
            ]
        }
    ]
...
external_gradient_fn_per_subexperiment += [
    lambda x: 0.25 * ((x - x_offset) / (WIDTH_MIGR_CORRIDOR))
]
parameter_override_dicts_per_sub_experiment += [
    [
...
external_gradient_fn_per_subexperiment += [
    lambda x: 0.25 * ((x - x_offset) / (WIDTH_MIGR_CORRIDOR)) ** 2
]
parameter_override_dicts_per_sub_experiment += [
    [
        update_pd_with_keyvalue_tuples(
...
# external_gradient_fn_per_subexperiment += [lambda x: 1.0*((x - x_offset)/(WIDTH_MIGR_CORRIDOR))**2]
...
if __name__ == "__main__":
    ...
    if RUN_EXPERIMENTS == True:
        global_scale = 1
        plate_height = HEIGHT_MIGR_CORRIDOR * 1.4
        plate_width = WIDTH_MIGR_CORRIDOR * 1.2
        ...
        animation_settings = dict(
            [
                ...
                ("velocity_scale", 1),
                ("rgtpase_scale", global_scale * 62.5 * 5),
