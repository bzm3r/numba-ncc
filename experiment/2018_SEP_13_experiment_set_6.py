...
if __name__ == '__main__':
  ...
  st = time.time()
  ets.chemotaxis_threshold_test_magnitudes(date_str, experiment_number, sub_experiment_number, copy.deepcopy(parameter_dict), no_randomization=False, base_output_dir="B:\\numba-ncc\\output\\", total_time_in_hours=10., timestep_length=2, verbose=True, integration_params=integration_params, max_timepoints_on_ram=10, seed=None, allowed_drift_before_geometry_recalc=allowed_drift_before_geometry_recalc, test_x_offset_in_corridor=625.0, test_chemo_magnitudes=[0.0], test_chemo_slope=0.02/40.0, num_experiment_repeats=5, timesteps_between_generation_of_intermediate_visuals=None, produce_graphs=True, produce_animation=True, full_print=True, delete_and_rerun_experiments_without_stored_env=True, run_experiments=True, remake_graphs=False, remake_animation=False, default_coa=coa_dict[2], default_cil=default_cil, chemotaxis_target_radius=160.0, box_width=1, box_height=1, box_y_placement_factor=0.5, num_cells=1)
  et = time.time()
  print("time taken: {}s".format(np.round(et - st, decimals=2)))
