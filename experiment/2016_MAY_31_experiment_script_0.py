...
biased_rgtpase_distrib_defn_dicts += [[{0: ['unbiased random', np.array([-np.pi/4, np.pi/4]) + np.pi/32 + np.pi, 0.2], 1: ['unbiased random', -1*(np.array([-np.pi/4, np.pi/4]) + np.pi/32 + np.pi), 0.2]}]]
