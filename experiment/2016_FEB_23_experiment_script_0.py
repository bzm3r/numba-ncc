...
cell_dependent_coa_signal_strengths_defn_dicts_per_sub_experiment = [[dict([(x, coa_strength*z) for x in boxes]) for y in boxes] for z in coa_strength_per_sub_experiment]

intercellular_contact_factor_magnitudes_defn_dicts_per_sub_experiment = [[dict([(x, cil_strength*z) for x in boxes]) for y in boxes] for z in cil_strength_per_sub_experiment]
...
for si in range(num_sub_experiments):
    ...
    environment_name_format_strings.append( form_base_environment_name_format_string(EXPERIMENT_NUMBER, np.sum(NUM_CELLS_IN_BOXES), str(np.round(TOTAL_TIME/(3600.), decimals=2))+"h", NUM_TIMESTEPS, NUM_NODES_PER_CELL, cil_strength_per_sub_experiment[si], coa_strength_per_sub_experiment[si], HEIGHT_MIGR_CORRIDOR, WIDTH_MIGR_CORRIDOR))
...
if __name__ == '__main__':
    ...
    if RUN_EXPERIMENTS == True:
        ...
        run_experiments(experiment_dir, environment_name_format_strings, environment_wide_variable_defns, user_cell_group_defns_per_subexperiment, experiment_descriptions_per_subexperiment, num_experiment_repeats=NUM_EXPERIMENT_REPEATS, elapsed_timesteps_before_producing_intermediate_graphs=2000, elapsed_timesteps_before_producing_intermediate_animations=2000, animation_settings=animation_settings, produce_intermediate_visuals=True, produce_final_visuals=True, full_print=True, elapsed_timesteps_before_hard_saving_env=None, elapsed_timesteps_before_soft_saving_env=50, delete_and_rerun_experiments_without_stored_env=True)
