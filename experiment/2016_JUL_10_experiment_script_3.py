...
plate_width, plate_height = 1000, 1000#1500, (box_heights[0] + physical_bdry_polygon_extra*2)*2

x_space_between_boxes = [2*CELL_DIAMETER, 2*CELL_DIAMETER, 2*CELL_DIAMETER]

x_offset, y_offset = 0.5*(plate_width - 0.5*box_widths[1]), 0.5*(plate_height - 0.5*box_heights[1])
...
space_migratory_bdry_polygon, space_physical_bdry_polygon = eu.make_space_polygons(make_migr_poly, make_phys_poly, WIDTH_MIGR_CORRIDOR, HEIGHT_MIGR_CORRIDOR, x_offset, y_offset, physical_bdry_polygon_extra=physical_bdry_polygon_extra)
...
biased_rgtpase_distrib_defn_dicts += [[{'default': ['unbiased random', np.array([0, 2*np.pi]), 0.3]}]]
external_gradient_fn_per_subexperiment += [lambda x: 0.0]
parameter_override_dicts_per_sub_experiment += [[eu.update_pd_with_keyvalue_tuples(parameter_override_dicts_per_sub_experiment[0][0], [('randomization', False, True)]), eu.update_pd_with_keyvalue_tuples(base_parameter_dict, [('skip_dynamics', False, True)])]]
...
biased_rgtpase_distrib_defn_dicts += [[{'default': ['unbiased random', np.array([0, 2*np.pi]), 0.3]}]]
external_gradient_fn_per_subexperiment += [lambda x: 0.0]
parameter_override_dicts_per_sub_experiment += [[eu.update_pd_with_keyvalue_tuples(parameter_override_dicts_per_sub_experiment[0][0], [('randomization', False, True), ('randomization_time_variance_factor', 0.25, 1e-2)]), eu.update_pd_with_keyvalue_tuples(base_parameter_dict, [('skip_dynamics', False, True)])]]
