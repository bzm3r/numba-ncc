...
all_po_sets = [
    {
        "randomize_rgtpase_distrib": 0.0,
        "kdgtp_rac_multiplier": 20.0,
        "kgtp_rac_multiplier": 77.171717171717162,
        "kgtp_rho_multiplier": 123.23232323232322,
        "kdgtp_rho_multiplier": 20.0,
        "kgtp_rho_autoact_multiplier": 500.0,
        "randomization_exponent": 1.0,
...
for po_set_index, candidate_po_set in enumerate(all_po_sets):
    ...
    datavis.graph_parameter_overrides(
        base_environment_name,
        candidate_po_set,
        save_name=base_environment_name + "_parameter_vis",
        save_dir=environment_dir,
    )
    ...
    for x in range(NUM_EXPERIMENT_REPEATS):
        ...
        symmetry_results = dict(
            list(zip(symmetry_output_labels, symmetry_output_values))
        )

        datavis.graph_important_cell_variables_over_time(
            a_cell,
            symmetry_ratings_dict=symmetry_results,
            save_name=environment_name + "_important_cell_vars_graph",
            save_dir=environment_dir,
        )

        datavis.graph_rates(
            a_cell,
            save_name=environment_name + "_strains_and_rates_graph",
            save_dir=environment_dir,
        )
        ...
        animation_obj = animator.EnvironmentAnimation(
            ...
            width_in_pixels,
            an_environment,
            origin_offset_in_micrometers,
            cartesian_to_pixels_scale=scale,
            rgtpase_info_scale=scale * 15,
            velocity_info_scale=scale,
            plot_rgtpase_lines=True,
            plot_forces=False,
        )
