...
if __name__ == '__main__':
  ...
  x_offset_in_corrdior = 625.0
  st = time.time()
  ets.single_cell_chemotaxis_virtual_size_test(date_str, experiment_number, sub_experiment_number, copy.deepcopy(parameter_dict), no_randomization=False, base_output_dir="B:\\numba-ncc\\output\\", total_time_in_hours=10., timestep_length=2, verbose=True, integration_params=integration_params, max_timepoints_on_ram=10, seed=None, allowed_drift_before_geometry_recalc=allowed_drift_before_geometry_recalc, test_x_offset_in_corridor=x_offset_in_corrdior, test_chemo_magnitude=2.75, test_chemo_slope=slope, num_experiment_repeats=50, timesteps_between_generation_of_intermediate_visuals=None, produce_graphs=True, produce_animation=False, full_print=True, delete_and_rerun_experiments_without_stored_env=True, run_experiments=True, remake_graphs=False, remake_animation=False, default_coa=coa_dict[1], default_cil=default_cil, chemotaxis_target_radius=160.0, box_y_placement_factor=0.5, test_virtual_size_factors=[0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0], skip_simulation_check=False)
  et = time.time()
