...
analysis_dir = exec_utils.get_analysis_directory_path(
    BASE_OUTPUT_DIR, DATE_STR, ANALYSIS_NUMBER
)

relevant_experiment_info = [("2016_MAR_28", 0, x) for x in [0, 1, 2]]

environment_dirs = exec_utils.get_environment_dirs_given_relevant_experiment_info(
...
def make_centroid_drift_and_delaunay_plots(
    environment_dirs, relevant_experiment_info, save_dir
):
    x_scale_factor = 400
    ...
    for i, environment_dir in enumerate(environment_dirs):
        ...
        stored_env = exec_utils.retrieve_environment(
            os.path.join(environment_dir, "environment.pkl"), False, False
        )
        ...
        all_cell_centroids_per_tstep = np.zeros(
            (max_tstep, num_cells, 2), dtype=np.float64
        )
        ...
        for ci in range(num_cells):
            cell_centroids_per_tstep = (
                analysis_utils.calculate_cell_centroids_until_tstep(
        ...
        delaunay_triangulations_per_tstep = [
            space.Delaunay(all_cell_centroids)
            for all_cell_centroids in all_cell_centroids_per_tstep
        ]
        ...
        normalized_convex_hull_areas = np.array(
            convex_hull_areas / convex_hull_areas[0]
        )
        timepoints = np.arange(normalized_convex_hull_areas.shape[0]) * T
        normalized_convex_hull_areas_and_tsteps_per_env.append(
            (timepoints, normalized_convex_hull_areas)
        )
        # ------------------------
        ...
        group_centroid_per_tstep = (
            np.array(
                [
                    geometry.calculate_cluster_centroid(cell_centroids)
        ...
        group_centroids_and_tsteps_per_env.append(
            (timepoints, group_centroid_per_tstep[:, 0])
        )

    fig, ax = plt.subplots()

    for exp_info, gcs_and_tsteps in zip(
        relevant_experiment_info, group_centroids_and_tsteps_per_env
    ):
    ...
    if save_dir == None:
        ...
        fig.savefig(
            os.path.join(save_dir, "collated_group_centroid" + ".png"), forward=True
        )
        plt.close(fig)
        plt.close("all")
    ...
    for exp_info, nas_and_tsteps in zip(
        relevant_experiment_info, normalized_convex_hull_areas_and_tsteps_per_env
    ):
        date_str, exp_number, sub_exp_number = exp_info
    ...
    if save_dir == None:
        ...
        fig.set_size_inches(12, 8)
        fig.savefig(
            os.path.join(save_dir, "collated_group_area" + ".png"), forward=True
        )
        plt.close(fig)
        plt.close("all")


make_centroid_drift_and_delaunay_plots(
    environment_dirs, relevant_experiment_info, save_dir
)
