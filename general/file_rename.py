...
def rename_experiment(sub_dir, current_dir, original_st, final_st, trial_run=False):
    ...
    sub_experiment_index, randomization_tag, total_num_cells, group_width, group_height, group_height, box_y_placement_factor, coa, cil, repeat_number = (
        parse_result_obj.fixed
    )

    new_sub_dir = final_st.format(
...
num_expected_blanks = len(
    (parse.parse(original_string_template, original_string_template)).fixed
)
...
for folder in sub_folders:
    ...
    if parse_result_obj != None:
        ...
        sub_experiment_index, randomization_tag, total_num_cells, group_width, group_height, group_height, box_y_placement_factor, coa, cil = (
            parse_result
        )
        new_dir_name = final_string_template.format(
            sub_experiment_index,
            randomization_tag,
            total_num_cells,
            group_width,
        ...
        for dir_item in current_dir_items:
            ...
            rename_experiment(
                sub_dir,
                current_dir,
                original_string_template + "_RPT={}",
                final_string_template + "_RPT={}",
                trial_run=trial_run,
            )
