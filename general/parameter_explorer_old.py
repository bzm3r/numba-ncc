...
def modify_pd_deterministically(
    parameter_dict, moddable_parameter_labels, mod_deltas, justifications, resolution
):
    modded_pds_per_change = []

    for mpl, mod_delta, sorted_j in zip(
        moddable_parameter_labels, mod_deltas, justifications
    ):
...
def generate_random_update_based_on_current_state(
    ...
    for mpl, delta, this_min_max in zip(
        ...
        for n in range(num_new_updates):
            new_value = (
                ...
                * np.random.choice([1, 1, 1, 1, 1, 2, 2, 2, 2, 5, 5, 5, 10, 10, 100])
                * delta
            )
        ...
        for n in range(num_experiment_repeats):
            ...
            task_list.append(
                exptempls.setup_polarization_experiment(
                    trial_dict, total_time_in_hours=total_time_in_hours, seed=seeds[n]
                )
            )
...
def rate_results_and_find_best_update(
    ...
    for n in range(len(scores_and_updates_no_randomization)):
        ...
        if scores_and_updates_with_randomization != None:
            ...
            combined_score = np.average(
                combined_score_with_randomization, combined_score_no_randomization
            )
        ...
        if combined_score > current_best_score:
            #print(
                "possible new score: {}, {}, {}".format(
                    ...
                    np.round(score_with_randomization, decimals=2),
                )
            )
    ...
    return new_best_score, new_best_update, new_dict, improvement, best_updates
...
def parameter_explorer_polarization_wanderer(
    ...
    current_dict.update(
        generate_random_starting_update(
    ...
    #
    #    current_dict_with_randomization = None
    #    if default_dict_has_randomization_scheme:
    #        current_dict_no_randomization.update([('randomization_scheme', None)])
    #        current_dict_with_randomization = copy.deepcopy(STANDARD_PARAMETER_DICT)
    #
    #    current_best_update_no_randomization = generate_random_starting_update(mod_labels, mod_deltas, mod_min_max, current_dict_no_randomization)
    #    current_dict_no_randomization.update(current_best_update_no_randomization)
    #
    #    if default_dict_has_randomization_scheme:
    #        current_best_update_with_randomization = generate_random_starting_update(mod_labels, mod_deltas, mod_min_max, current_dict_with_randomization)
    #        current_dict_with_randomization.update(current_best_update_with_randomization)
    #
    #    init_cell_no_randomization = executils.run_simple_experiment_and_return_cell_worker(exptempls.setup_polarization_experiment(current_dict_no_randomization, total_time_in_hours=total_time_in_hours))
    #
    #    init_cell_with_randomization = None
    #    if default_dict_has_randomization_scheme:
    #        init_cell_with_randomization = executils.run_simple_experiment_and_return_cell_worker(exptempls.setup_polarization_experiment(current_dict_with_randomization, total_time_in_hours=total_time_in_hours))
    #
    #    current_scores = cu.calculate_parameter_exploration_score_from_cell(init_cell_no_randomization, significant_difference=0.2, weigh_by_timepoint=False, init_cell_with_randomization=init_cell_with_randomization)
    #    current_best_score = calculate_score(current_scores)
    ...
    while not stop_criterion_met:
        ...
        trial_updates = generate_random_update_based_on_current_state(
        ...
        current_best_score, current_best_update, current_dict, improvement = rate_results_and_find_best_update(
            current_best_score,
            current_dict,
            scores_and_updates_no_randomization,
            best_updates,
...
def parameter_explorer_polarization_slope_follower(
    modification_program,
    required_polarization_score,
    num_new_dicts_to_generate=8,
    task_chunk_size=4,
    num_processes=4,
    sequential=False,
    ...
    current_best_update = generate_random_starting_update(
        mod_labels, mod_deltas, mod_min_max, current_dict
    )  # [(label, lims[0]) for label, lims in zip(mod_labels, mod_min_max)]
    current_dict.update(current_best_update)

    current_polarization_score = cu.calculate_rgtpase_polarity_score_from_cell(
        executils.run_simple_experiment_and_return_cell_worker(
            exptempls.setup_polarization_experiment(
                current_dict, total_time_in_hours=total_time_in_hours
            )
        ),
    ...
    while not stop_criterion_met:
        ...
        dprs = []
        for label_index, loop_data in enumerate(
            zip(mod_labels, mod_min_max, mod_deltas)
        ):
            ...
            seeds = [
                int(np.round(np.random.rand(), decimals=4) * 1000)
                for n in range(num_experiment_repeats)
            ]
            task_list = [
                exptempls.setup_polarization_experiment(
                    x, seed=seeds[n], total_time_in_hours=total_time_in_hours
                )
                for n in range(num_experiment_repeats)
            ] + [
                exptempls.setup_polarization_experiment(
                    y, seed=seeds[n], total_time_in_hours=total_time_in_hours
                )
                for n in range(num_experiment_repeats)
            ]
            ...
            x_pr = np.array(
                [
                    cu.calculate_rgtpase_polarity_score_from_cell(
                        rc, significant_difference=0.2, weigh_by_timepoint=False
                    )[0]
                    for rc in x_cells
                ]
        ...
        dprs = np.array([bounded(v, [-1.0, 1.0]) for v in dprs])
        ...
        new_values = np.array(
            [
                bounded(v + w * d, lims)
                for v, w, d, lims in zip(old_values, dprs, mod_deltas, mod_min_max)
            ]
        )
        ...
        for old_cbu, new_cbu in zip(old_update, current_best_update):
            print(
                "{}: {}, {}".format(
                    old_cbu[0],
                    np.round(new_cbu[1] / old_cbu[1], decimals=3),
                    new_cbu[1],
                )
            )
        ...
        task_list = [
            exptempls.setup_polarization_experiment(current_dict)
            for n in range(num_experiment_repeats)
        ]
        result_cells = worker_pool.map(
            executils.run_simple_experiment_and_return_cell_worker, task_list
        )
        current_polarization_score = np.average(
            [
                cu.calculate_rgtpase_polarity_score_from_cell(
                    rc, significant_difference=0.2, weigh_by_timepoint=False
                )[0]
                for rc in result_cells
            ]
        )
...
def create_task_value_arrays(parameter_exploration_program, num_processes):
    ...
    for (
        ...
        end_value,
        range_resolution,
    ) in parameter_exploration_program:
        given_parameter_labels.append(parameter_label)
        given_parameter_values.append(
            np.floor(np.linspace(start_value, end_value, num=range_resolution))
        )

    all_parameter_labels = list(
        parameterorg.all_user_parameters_with_justifications.keys()
    )
    for parameter_label in given_parameter_labels:
        if parameter_label not in all_parameter_labels:
            raise Exception(
                "Parameter label {} not in accepted parameter dictionary.".format(
                    parameter_label
                )
            )
    ...
    chunky_task_value_array_indices = general_utils.chunkify(
        np.arange(num_combinations), num_processes
    )
...
def parameter_explorer_asymmetry_criteria(
    parameter_exploration_name,
    parameter_exploration_program,
    sequential=False,
    result_storage_folder="A:\\numba-ncc\\output",
    overwrite=False,
    seed=36,
    run=True,
    ...
    if stored_data_exists:
        ci_offset = hardio.get_last_executed_parameter_exploration_chunk_index(
            storefile_path
        )
        ...
        if stored_parameter_exploration_program != parameter_exploration_program:
            raise Exception(
                "Stored exploration program does not match given exploration program! stored: {} || given: {}".format(
                    stored_parameter_exploration_program, parameter_exploration_program
                )
            )

        given_parameter_labels, given_parameter_values, task_value_arrays, chunky_task_value_array_indices = create_task_value_arrays(
            parameter_exploration_program, num_processes
        )
    else:
        ci_offset = 0
        ...
        hardio.create_parameter_exploration_dataset(
            storefile_path, len(given_parameter_labels) + 1
        )
    ...
    for ci, task_index_chunk in enumerate(chunky_task_value_array_indices[ci_offset:]):
        ...
        results = np.zeros(
            (num_processes, len(given_parameter_labels) + 1), dtype=np.float64
        )
        ...
        for task_index in task_index_chunk:
            ...
            task_environment_defn = exptempls.setup_polarization_experiment(
                parameter_dict,
                seed=seed,
                init_rho_gtpase_conditions=init_rho_gtpase_conditions,
            )
        ...
        if sequential == True:
            ...
            for task in task_chunk:
                loop_result_cells.append(
                    executils.run_simple_experiment_and_return_cell_worker(task)
                )
        else:
            loop_result_cells = worker_pool.map(
                executils.run_simple_experiment_and_return_cell_worker, task_chunk
            )
        ...
        results = np.array(
            [
                np.append(
                    ...
                    task_value_arrays[ti],
                )
                for a_cell, ti in zip(loop_result_cells, task_index_chunk)
            ]
        )
        ...
        hardio.append_parameter_exploration_data_to_dataset(
            ci + ci_offset, results, storefile_path
        )
...
if __name__ == "__main__":
    ...
    moddable_parameters = [
        ("kgtp_rac_multiplier", 1.0, 500.0, 1.0),
        ("kgtp_rho_multiplier", 1.0, 500.0, 1.0),
        ("kdgtp_rac_multiplier", 1.0, 500.0, 1.0),
        ("kdgtp_rho_multiplier", 1.0, 500.0, 1.0),
        ("threshold_rac_activity_multiplier", 0.1, 0.8, 0.01),
        ("threshold_rho_activity_multiplier", 0.1, 0.8, 0.01),
        ("kgtp_rac_autoact_multiplier", 1.0, 1000.0, 1.0),
        ("kgtp_rho_autoact_multiplier", 1.0, 1000.0, 1.0),
        ("kdgtp_rac_mediated_rho_inhib_multiplier", 1.0, 2000.0, 1.0),
        ("kdgtp_rho_mediated_rac_inhib_multiplier", 1.0, 2000.0, 1.0),
        ("tension_mediated_rac_inhibition_half_strain", 0.01, 0.5, 0.1),
        ("stiffness_edge", 1000, 8000, 500),
        ("max_force_rac", 0.5 * 10e3, 5 * 10e3, 0.1 * 10e3),
        ("eta", 0.25 * 1e5, 9 * 1e5, 0.1 * 1e5),
        ("randomization_time_mean", 1.0, 40.0, 2.0),
        ("randomization_time_variance_factor", 0.1, 0.5, 0.1),
        ("randomization_magnitude", 2.0, 20.0, 1.0),
    ]
    ...
    best_updates = parameter_explorer_polarization_wanderer(
        moddable_parameters,
        0.8,
        num_new_dicts_to_generate=int(len(moddable_parameters) * 0.5),
        num_experiment_repeats=3,
        num_processes=4,
        total_time_in_hours=3.0,
        seed=2836,
