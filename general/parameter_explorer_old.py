...
def rate_results_and_find_best_update(current_best_score, current_dict, scores_and_updates_no_randomization, best_updates, scores_and_updates_with_randomization=None):
...
def parameter_explorer_polarization_wanderer(modification_program, required_score, num_new_dicts_to_generate=8, task_chunk_size=4, num_processes=4, num_experiment_repeats=3, num_experiment_repeats_no_randomization=1, total_time_in_hours=2., seed=None, sequential=False):
    ...
    current_dict.update(generate_random_starting_update(mod_labels, mod_deltas, mod_min_max, current_dict))
...
#    best_update = get_result_as_dict_update(sorted_results, -1, p_labels)
    ...
    best_updates = parameter_explorer_polarization_wanderer(moddable_parameters, 0.8, num_new_dicts_to_generate=int(len(moddable_parameters)*0.5), num_experiment_repeats=3, num_processes=4, total_time_in_hours=3., seed=2836, sequential=True)
