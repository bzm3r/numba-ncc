...
dist_squared_matrix0, line_segment_intersect_matrix0 = singlethread_func(dist_squared_and_line_segment_calculation_worker, num_cells, num_nodes, polygon_bounding_boxes, polygons)
...
st = time.time()
for n in range(10):
    dist_squared_matrix, line_segment_intersect_matrix = multithread_func(dist_squared_and_line_segment_calculation_worker, nthreads, num_cells, num_nodes, polygon_bounding_boxes, polygons)
et = time.time()

print("avg. time to run multithread_func: ", (et - st)/num_repeats)

print("dist_squared_matrix agrees: ", np.all(np.abs(dist_squared_matrix - dist_squared_matrix0) < 1e-12))

print("line_segment_intersect_matrix agrees: ", np.all(np.abs(line_segment_intersect_matrix - line_segment_intersect_matrix0) == 0))
