...
def generate_displacements(persistence_time, num_displacements, timestep_bw_displacements, avg_displacement_magnitude):
    ...
    for n in range(num_displacements):
        ...
        if np.random.rand() < 1. - np.exp(-delta_t/persistence_time):
            theta = np.random.rand()*2*np.pi
            delta_t = 0.0
...
def estimate_persistence_time(timestep, positive_das):
    ...
    popt, pcov = sp.optimize.curve_fit(lambda t, pt: np.exp(-1.*t/pt), ts, positive_das)
    pt = popt[0]
...
all_das, positive_das = calculate_direction_autocorr_coeffs_for_persistence_time(displacements)
