...
def inner_func_nb(dist_squared_matrix, line_segment_intersect_matrix, polygon_bounding_boxes, polygons, task_addresses):
    """
    Function under test.
    """
...
@nb.jit('void(int64[:], int64[:,:], float64[:,:], float64[:,:,:])', nopython=True, nogil=True)
def line_segment_intersection_calculation_worker(line_segment_intersection_results, task_addresses, polygon_bounding_boxes, polygons):
    """
    Function under test.
    """
    ...
    for n in range(task_addresses.shape[0]):
        ...
        line_segment_intersection_results[n] = check_if_line_segment_going_from_vertex_of_one_polygon_to_vertex_of_another_passes_through_any_polygon(a_ci, a_ni, b_ci, b_ni, polygons, polygon_bounding_boxes)
        
@nb.jit('void(float64[:], int64[:,:], float64[:,:,:])', nopython=True, nogil=True)
def dist_squared_calculation_worker(dist_squared_results, task_addresses, polygons):
    """
    Function under test.
    """
    ...
    for n in range(task_addresses.shape[0]):
        a_ci, a_ni, b_ci, b_ni = task_addresses[n][0], task_addresses[n][1], task_addresses[n][2], task_addresses[n][3]
        dist_squared_results[n] = calculate_squared_dist(polygons[a_ci][a_ni], polygons[b_ci][b_ni])

@nb.jit(nopython=True, nogil=True)
def create_dist_and_line_segment_interesection_test_args(num_cells, num_nodes_per_cell, init_cells_bounding_box_arrays, init_all_cells_node_coords):
...
@nb.jit(nopython=True, nogil=True)
def store_results_into_matrices(line_segment_intersect_results, dist_squared_results, tasks, dist_squared_matrix, line_segment_intersect_matrix):
    for n in range(tasks.shape[0]):
        address = tasks[n]
        a_ci, a_ni, b_ci, b_ni = address[0], address[1], address[2], address[3]
        dist_squared_result = dist_squared_results[n]
        dist_squared_matrix[a_ci][a_ni][b_ci][b_ni] = dist_squared_result
        dist_squared_matrix[b_ci][b_ni][a_ci][a_ni] = dist_squared_result
        
        line_segment_intersect_result = line_segment_intersect_results[n]
        line_segment_intersect_matrix[a_ci][a_ni][b_ci][b_ni] = line_segment_intersect_result
        line_segment_intersect_matrix[b_ci][b_ni][a_ci][a_ni] = line_segment_intersect_result
        
def singlethread_func(inner_func, num_cells, num_nodes, polygon_bounding_boxes, polygons):
    """
    Run the given function inside a single thread.
    """
    
    tasks = create_dist_and_line_segment_interesection_test_args(num_cells, num_nodes, polygon_bounding_boxes, polygons)
    ...
    inner_func(dist_squared_matrix, line_segment_intersect_matrix, polygon_bounding_boxes, polygons, tasks)
    
    return dist_squared_matrix, line_segment_intersect_matrix

def multithread_func(inner_func, num_threads, num_cells, num_nodes, polygon_bounding_boxes, polygons):
    """
    ...
    tasks = np.array(tasks, dtype=np.int64)
    num_tasks = len(tasks)
    line_segment_intersection_results = -1*np.ones(tasks.shape[0], dtype=np.int64)
    dist_squared_results = -1*np.ones(tasks.shape[0], dtype=np.float64)
    
    chunklen = (num_tasks + num_threads - 1) // num_threads
    line_segment_intersection_args = (line_segment_intersection_results,) + (tasks,)
    dist_squared_args = (dist_squared_results,) + (tasks,)
    
    # Create argument tuples for each input chunk
    line_segment_intersection_chunks = []
    ...
    threads = [threading.Thread(target=line_segment_intersection_calculation_worker, args=c)
               for c in line_segment_intersection_chunks]
    ...
    threads = [threading.Thread(target=dist_squared_calculation_worker, args=c)
               for c in dist_squared_chunks]
    ...
    store_results_into_matrices(line_segment_intersection_results, dist_squared_results, tasks, dist_squared_matrix, line_segment_intersect_matrix)
...
def generate_polygons(num_polygons, num_vertices_per_polygon, max_radius=1., min_max_origin=[0., 5.]):
...
polygon_bounding_boxes = create_initial_bounding_box_polygon_array(num_cells, num_nodes, polygons)

dist_squared_matrix0, line_segment_intersect_matrix0 = singlethread_func(inner_func_nb, num_cells, num_nodes, polygon_bounding_boxes, polygons)
...
st = time.time()
for n in range(num_repeats):
    dist_squared_matrix1, line_segment_intersect_matrix1 = multithread_func(inner_func_nb, nthreads, num_cells, num_nodes, polygon_bounding_boxes, polygons)
et = time.time()

print("avg. time to run multithread_func: ", (et - st)/num_repeats)

print("dist_squared_matrix agrees: ", np.all(np.abs(dist_squared_matrix1 - dist_squared_matrix0) < 1e-12))

print("line_segment_intersect_matrix agrees: ", np.all(np.abs(line_segment_intersect_matrix1 - line_segment_intersect_matrix0) == 0))
