...
def line_segment_intersection_calculation_worker(
    line_segment_intersection_results, task_addresses, polygon_bounding_boxes, polygons
):
    """
    Function under test.
    """
    ...
    for n in range(task_addresses.shape[0]):
        ...
        line_segment_intersection_results[
            n
        ] = check_if_line_segment_going_from_vertex_of_one_polygon_to_vertex_of_another_passes_through_any_polygon(
            a_ci, a_ni, b_ci, b_ni, polygons, polygon_bounding_boxes
        )
...
def dist_squared_calculation_worker(dist_squared_results, task_addresses, polygons):
    ...
    for n in range(task_addresses.shape[0]):
        ...
        dist_squared_results[n] = calculate_squared_dist(
            polygons[a_ci][a_ni], polygons[b_ci][b_ni]
        )
...
#@nb.jit(nopython=True, nogil=True)
def store_results_into_matrices(
    line_segment_intersect_results,
    dist_squared_results,
    tasks,
    dist_squared_matrix,
    line_segment_intersect_matrix,
):
    for n in range(tasks.shape[0]):
        ...
        line_segment_intersect_matrix[a_ci][a_ni][b_ci][
            b_ni
        ] = line_segment_intersect_result
        line_segment_intersect_matrix[b_ci][b_ni][a_ci][
            a_ni
        ] = line_segment_intersect_result
...
def multithread_func(
    ...
    threads = [
        threading.Thread(target=line_segment_intersection_calculation_worker, args=c)
...
dist_squared_matrix0, line_segment_intersect_matrix0 = singlethread_func(
    inner_func_nb, num_cells, num_nodes, polygon_bounding_boxes, polygons
)
...
st = time.time()
for n in range(num_repeats):
    dist_squared_matrix1, line_segment_intersect_matrix1 = multithread_func(
        inner_func_nb, nthreads, num_cells, num_nodes, polygon_bounding_boxes, polygons
    )
et = time.time()
...
print(
    ...
    np.all(np.abs(dist_squared_matrix1 - dist_squared_matrix0) < 1e-12),
)

print(
    "line_segment_intersect_matrix agrees: ",
    np.all(
        np.abs(line_segment_intersect_matrix1 - line_segment_intersect_matrix0) == 0
    ),
)
