...
def are_points_inside_polygon(num_points, points, num_poly_vertices, polygon):
    ...
    for index in range(num_points):
        ...
        results[index] = is_point_in_polygon_given_bounding_box(
            test_point, num_poly_vertices, polygon, min_x, max_x, min_y, max_y
        )
...
def generate_cell_coords(num_nodes, r, ox, oy):
    return np.array(
        [
            [ox + r * np.cos(theta), oy + r * np.sin(theta)]
            for theta in np.linspace(0, 2 * np.pi, num=num_nodes, endpoint=False)
        ]
    )
...
cc0 = generate_cell_coords(num_nodes, 1.0, 0.0, 0.0)
cc1 = generate_cell_coords(num_nodes, 0.5, 0.0, 0.0)

all_cells_node_coords = np.array([cc0, cc1])

result = check_if_nodes_inside_other_cells(
    1, num_nodes, num_cells, all_cells_node_coords
)
