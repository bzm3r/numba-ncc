...
with h5py.File(t0, "a") as f:
    dset = f.create_dataset("test", (0, num_vertices, num_info), maxshape=(None, num_vertices, num_info), compression="gzip", compression_opts=9)

for ix in np.arange(int(num_timepoints/output_size)):
    chunk = np.copy(np.copy(data[(ix*output_size):(ix*output_size + output_size)]))
    with h5py.File(t0, "a") as f:
        dset = f["test"]
