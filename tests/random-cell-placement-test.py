...
def place_init_cell_randomly(cell_bounding_boxes, cell_diameter, corridor_origin, corridor_height, box_height, box_width):
    min_x = 0.25*cell_diameter*(box_width - 1)*np.random.rand() + corridor_origin[0]
    min_y = cell_diameter*(box_height - 1)*np.random.rand() + corridor_origin[1]
    
    cell_bounding_boxes[0] = np.array([min_x, min_x + cell_diameter, min_y, min_y + cell_diameter])
...
def is_collision(last_placed_cell_index, cell_bounding_boxes, cell_diameter, corridor_origin, corridor_height, test_min_x, test_min_y):
    ...
    if test_min_y > corridor_origin[1] + (corridor_height - 1)*cell_diameter or test_min_y < corridor_origin[1]:
        return True
...
def try_placing_cell_randomly(last_successful_anchor_index, last_placed_cell_index, cell_bounding_boxes, theta_bins, cell_diameter, corridor_origin, corridor_height, box_height, box_width, min_placement_dist, max_placement_dist):
    ...
    for ti in xrange(num_trials):
        ...
        if is_collision(last_placed_cell_index, cell_bounding_boxes, cell_diameter, corridor_origin, corridor_height, test_min_x, test_min_y):
            ...
            cell_bounding_boxes[cell_index] = np.array([test_min_x, test_min_x + cell_diameter, test_min_y, test_min_y + cell_diameter])
...
def place_cells_randomly(num_cells, cell_diameter, corridor_origin, corridor_height, box_height, box_width, min_placement_distance, max_placement_distance, num_theta_bins = 20):
    cell_bounding_boxes = np.nan*np.ones((num_cells, 4), dtype=np.float64)
    cell_bounding_boxes = place_init_cell_randomly(cell_bounding_boxes, cell_diameter, corridor_origin, corridor_height, box_height, box_width)
    ...
    while num_cells_placed != num_cells:
        cell_index, cell_bounding_boxes = try_placing_cell_randomly(possible_anchor_indices[trial_anchor_index], num_cells_placed - 1, cell_bounding_boxes, theta_bins, cell_diameter, corridor_origin, corridor_height, box_height, box_width, min_placement_distance, max_placement_distance)
...
def generate_centers_from_bounding_boxes(bounding_boxes):
    ...
    for i, bb in enumerate(bounding_boxes):
        centers[i] = np.array([np.average(bounding_boxes[i][:2]), np.average(bounding_boxes[i][2:])])
