...
def graph_centroid_related_data(skip_dynamics_flags, num_cells, num_timepoints, T, time_unit, cell_Ls, storefile_path, save_dir=None, save_name=None, max_tstep=None, make_group_centroid_drift_graph=True, fontsize=22, general_data_structure=None):    
...
def graph_important_cell_variables_over_time_old(T, L, cell_index, tension_mediated_rac_inhibition_half_strain, storefile_path, polarity_scores=None, save_dir=None, save_name=None, max_tstep=None, fontsize=22, general_data_structure=None, convergence_test=False, make_animation=True):
    set_fontsize(fontsize)
    fig, ax = plt.subplots()
    ...
    global_strains = (hardio.get_data_until_timestep(cell_index, max_tstep, 'global_strain', storefile_path)[:, 0])/tension_mediated_rac_inhibition_half_strain
    ...
    for data_set, line_style, data_label in zip([sum_rac_act_over_nodes, sum_rho_act_over_nodes, sum_rac_inact_over_nodes, sum_rho_inact_over_nodes, rac_cyt_gdi, rho_cyt_gdi, global_strains], ['b', 'r', 'b--', 'r--', 'c', 'm', 'k'], ['rac_active', 'rho_active', 'rac_inactive', 'rho_inactive', 'rac_gdi', 'rho_gdi', 'global_strain']):
...
def graph_edge_and_areal_strains(T, cell_index, storefile_path, save_dir=None, save_name=None, max_tstep=None, fontsize=22, general_data_structure=None):
    ...
    time_points = T*np.arange(global_strains.shape[0])
    
    #ax_e.plot(time_points, avg_edge_strains, 'k', label='avg_local_strains')
    #ax_e.plot(time_points, avg_tensile_local_strains, 'r', label='avg_tensile_local_strains')
    ax_e.plot(time_points, global_strains, 'k', label='global_strain')
...
def present_collated_single_cell_motion_data(centroids_persistences_speeds_per_repeat, experiment_dir, total_time_in_hours, time_unit, fontsize=22, general_data_structure=None):
    ...
    if experiment_dir != None:
        write_lines_to_file(os.path.join(experiment_dir, "single_cell_persistence_data.txt"), ["P_R over {} hours (mean: {}, std: {})\nP_T mean: {} {}, (std {} {})\ncell x:P_R, P_T".format(total_time_in_hours, mean_persistence_ratio, std_persistence_ratio, mean_persistence_time, time_unit, std_persistence_time, time_unit)] + ["cell {}: {}, {}\n".format(ci, pr, pt) for ci, pr, pt in zip(np.arange(len(persistence_times)), persistence_ratios, persistence_times)])
    else:
...
def graph_coa_variation_test_data_simple(sub_experiment_number, num_timepoints, T, num_cells_to_test, test_coas, average_cell_group_area_data, save_dir=None, general_data_structure=None):
    set_fontsize(fontsize)
    fig, ax = plt.subplots()
...
def calculate_statistics_and_write_into_general_file(text_file_name, labels, group_persistence_ratios, group_x_velocities, areal_strains, save_dir):
    lines = []
...
def graph_corridor_migration_parameter_test_data(labels, group_persistence_ratios, group_persistence_times, fit_group_x_velocities, cell_separations, areal_strains, experiment_set_label, save_dir=None, fontsize=22):
    set_fontsize(2*fontsize)
    #calculate_statistics_and_write_into_text_file(sub_experiment_number, test_num_cells, test_heights, group_persistence_ratios, fit_group_x_velocities, areal_strains, save_dir)
    ...
    for graph_type in ["box", "dot"]:
        ...
        for i, ds_label in enumerate(data_plot_order):
            ...
            show_or_save_fig(fig, (6, 6), save_dir, 'parameter_test_data', experiment_set_label + "_{}_{}".format(ds_label, graph_type))
        
        show_or_save_fig(fig_rows, (12, 3*len(data_plot_order)), save_dir, 'parameter_test_data', experiment_set_label + "_{}".format(graph_type))
            

#=======================================================================
        
def graph_coa_variation_test_data(sub_experiment_number, test_coas, default_cil, corridor_height, num_cells, group_persistence_ratios, group_persistence_times, fit_group_x_velocities, cell_separations, areal_strains, experiment_set_label, save_dir=None, fontsize=22):
    
    set_fontsize(2*fontsize)

    x_axis_positions = [i + 1 for i in range(len(test_coas))]
    x_axis_stuff = test_coas
    x_label = "$M_{COA}$"#\n($M_{CIL}$" + "={}, corridor height = {} cell diam., {} cells)".format(default_cil, corridor_height, num_cells)
    ...
    data_plot_order = ["average cell separation", "group X velocity", "migration intensity"]#["average cell separation", "group persistence ratios", "group persistence times", "group X velocity", "migration intensity"]
    ...
    for graph_type in ["box", "dot"]:
        ...
        for i, ds_label in enumerate(data_plot_order):
            ...
            show_or_save_fig(fig, (6, 6), save_dir, 'coa_variation_test', experiment_set_label + "_{}_{}".format(ds_label, graph_type))
        
        show_or_save_fig(fig_rows, (12*1.5, 3*len(data_plot_order)), save_dir, 'coa_variation_test', experiment_set_label + "_{}".format(graph_type))
        

#=======================================================================
        
def graph_cil_variation_test_data(sub_experiment_number, test_cils, default_coa, corridor_height, num_cells, group_persistence_ratios, group_persistence_times, fit_group_x_velocities, cell_separations, areal_strains, experiment_set_label, save_dir=None, fontsize=22):
    
    set_fontsize(2*fontsize)

    x_axis_positions = [i + 1 for i in range(len(test_cils))]
    x_axis_stuff = test_cils
    x_label = "$M_{CIL}$"#\n($M_{COA}$" + "={}, corridor height = {} cell diam., {} cells)".format(default_coa, corridor_height, num_cells)
    ...
    for graph_type in ["box", "dot"]:
        ...
        for i, ds_label in enumerate(data_plot_order):
            ...
            show_or_save_fig(fig, (6, 6), save_dir, 'cil_variation_test', experiment_set_label + "_{}_{}".format(ds_label, graph_type))
        
        show_or_save_fig(fig_rows, (12, 3*len(data_plot_order)), save_dir, 'cil_variation_test', experiment_set_label + "_{}".format(graph_type))
        
#=======================================================================
        
def graph_vertex_choice_variation_test_data(test_vertex_choice_ratios_and_randomization_magnitudes, default_cil, default_coa, corridor_height, num_cells, group_persistence_ratios, group_persistence_times, fit_group_x_velocities, cell_separations, areal_strains, experiment_set_label, save_dir=None, fontsize=22):
    
    set_fontsize(2*fontsize)

    x_axis_positions = [i + 1 for i in range(len(test_vertex_choice_ratios_and_randomization_magnitudes))]
    x_axis_stuff = ["{}".format(tvr) + "\n$x_r$={}".format(trm) for tvr, trm in test_vertex_choice_ratios_and_randomization_magnitudes]
    x_label = "ratio of randomly selected vertices to total vertices\n$M_{CIL}$"+"={}".format(default_cil) + ",($M_{COA}$" + "={}, corridor height = {} cell diam., {} cells)".format(default_coa, corridor_height, num_cells)
    ...
    for graph_type in ["box", "dot"]:
        ...
        for i, ds_label in enumerate(data_plot_order):
            ...
            show_or_save_fig(fig, (6, 6), save_dir, 'vertex_choice_variation', experiment_set_label + "_{}_{}".format(ds_label, graph_type))
        
        show_or_save_fig(fig_rows, (12, 3*len(data_plot_order)), save_dir, 'vertex_choice_variation', experiment_set_label + "_{}".format(graph_type))
...
def graph_init_condition_change_data(sub_experiment_number, tests, group_persistence_ratios, group_persistence_times, fit_group_x_velocities, cell_separations, transient_end_times, experiment_set_label, save_dir=None, fontsize=22):
    ...
    for graph_type in ["box", "dot"]:
        ...
        show_or_save_fig(fig_rows, (12, 3*len(data_plot_order)), save_dir, 'init_conditions_', experiment_set_label + "_{}".format(graph_type))
...
def determine_intercellular_separations_after_first_collision(all_cell_centroids_per_repeat, cell_diameter, pre, cutoff):
    ...
    for all_cell_centroids in all_cell_centroids_per_repeat:
        ...
        start = pre
        blanks = 0
        if timestep_at_first_collision < pre and pre != - 1:
            start = 0
            blanks = pre - timestep_at_first_collision
        elif pre == -1:
            start = timestep_at_first_collision
            
        end = timestep_at_first_collision + cutoff
        if end > ics_per_tstep.shape[0]:
            end = ics_per_tstep.shape[0]
        
        if blanks != 0:
            cell_intercellular_separations_after_first_collision.append((timestep_at_first_collision, np.append(np.nan*np.zeros(blanks, dtype=np.float64), ics_per_tstep[start:end])*cell_diameter))
        else:
            cell_intercellular_separations_after_first_collision.append((timestep_at_first_collision, ics_per_tstep[start:end]*cell_diameter))
...
#    
#    scarpa_time_data = [3.0, 8.0, 14.0, 19.0, 24.0, 30.0, 35.0, 40.0]
#    scarpa_nuclear_separation_data = [ 58.,  45.,  42.,  46.,  54.,  66.,  74.,  88.]
#    scarpa_collision_timepoint = scarpa_time_data[1]
#    
#    cell_intercellular_separations_after_first_collision = determine_intercellular_separations_after_first_collision(all_cell_centroids_per_repeat, cell_diameter, int(scarpa_collision_timepoint/T), int(40.0/T))
#    
#    fig, ax = plt.subplots()
...
#    
#    # plotting data from Scarpa et al., Figure 3g, 1D data
#    ax.plot(scarpa_time_data, scarpa_nuclear_separation_data, color='k', marker='o', markersize=5, ls='', label="Scarpa et al. observations")
#        
#    
#    ax.annotate('collision', xy=(scarpa_collision_timepoint, scarpa_nuclear_separation_data[1]), xytext=(scarpa_collision_timepoint, scarpa_nuclear_separation_data[1] + 25),
#            arrowprops=dict(facecolor='black', shrink=0.5*0.5*0.25, width=0.5*5),)
#    ax.axvline(x=scarpa_collision_timepoint, ls='dashed', color='k')
#
#    ax.set_xlabel("t (min.)")
...
#    
#    fig, ax = plt.subplots()
#    tpoint_of_interest = int((30.0 + scarpa_collision_timepoint)/T)
...
def graph_intercellular_distance_after_first_collision(all_cell_centroids_per_repeat, T, cell_diameter, save_dir=None):
    if all_cell_centroids_per_repeat[0].shape[0] != 2:
        raise Exception("Logic for determining post-collision separation distances for more than 2 cells has not been implemented! all_cell_centroids_per_repeat shape: {}".format(all_cell_centroids_per_repeat.shape))
    
    scarpa_time_data = np.array([8.0, 14.0, 19.0, 24.0, 30.0, 35.0, 40.0], dtype=np.float64) - 8.0
    scarpa_nuclear_separation_data = np.array([45.,  42.,  46.,  54.,  66.,  74.,  88.], dtype=np.float64)
    scarpa_collision_timepoint = scarpa_time_data[0]
    
    cell_intercellular_separations_after_first_collision = determine_intercellular_separations_after_first_collision(all_cell_centroids_per_repeat, cell_diameter, -1, int(40.0/T))
    ...
    ax.annotate('collision', xy=(scarpa_collision_timepoint, scarpa_nuclear_separation_data[1] + 5), xytext=(scarpa_collision_timepoint, scarpa_nuclear_separation_data[1] + 30),
            arrowprops=dict(facecolor='black', shrink=0.1, width=0.5*5),)
    #ax.axvline(x=scarpa_collision_timepoint, ls='dashed', color='k')
