...
def add_to_general_data_structure(general_data_structure, key_value_tuples):
    if general_data_structure != None:
        if type(general_data_structure) != dict:
            raise Exception("general_data_structure is not dict, instead: {}".format(type(general_data_structure)))
        else:
...
def graph_group_area_and_cell_separation_over_time_and_determine_subgroups(num_cells, num_nodes, num_timepoints, T,
                                                                           storefile_path, save_dir=None, fontsize=22,
    ...
    normalized_areas, normalized_cell_separations, cell_subgroups = cu.calculate_normalized_group_area_and_average_cell_separation_over_time(
        20, num_cells, num_timepoints, storefile_path, get_subgroups=graph_group_centroid_splits)
    ...
    if save_dir != None:
        write_lines_to_file(os.path.join(save_dir, "group_area_data.txt"),
    ...
    if save_dir != None:
        write_lines_to_file(os.path.join(save_dir, "group_separation_data.txt"),
                            ["group area (mean, deviation): {}, {}".format(mean, deviation)])
    else:
    ...
    if save_dir != None:
        write_lines_to_file(os.path.join(save_dir, "group_ratio_data.txt"),
                            ["group area (mean, deviation): {}, {}".format(mean, deviation)])
    else:
...
def graph_group_centroid_drift(T, time_unit, group_centroid_per_tstep, relative_all_cell_centroids_per_tstep, save_dir,
    ...
    transient_end_index = find_approximate_transient_end(group_x_velocities, average_group_x_velocity,
                                                         next_timesteps_window=60)
    ...
    if "cell_subgroups" in list(general_data_structure.keys()):
        ...
        if cell_subgroups != None:
                    ...
                    if last_num_subgroups != num_subgroups:
                        ...
                        for p in connected_subgroups:
                            if p[2] == "split":
                                ...
                                group_split_connector_data += [last_subgroup_centroids[p[1]], subgroup_centroids[p[0]],
                                                               np.nan]
                            else:
                                group_merge_connector_timesteps += [timepoints[ti - 1], timepoints[ti], np.nan]
                                group_merge_connector_data += [last_subgroup_centroids[p[1]], subgroup_centroids[p[0]],
                                                               np.nan]
            ...
            if save_dir == None or save_name == None:
                ...
                show_or_save_fig(fig, (6, 4), save_dir, "group_split_" + save_name, "", figtype="png")
...
def graph_centroid_related_data(skip_dynamics_flags, num_cells, num_timepoints, T, time_unit, cell_Ls, storefile_path,
                                save_dir=None, save_name=None, max_tstep=None, make_group_centroid_drift_graph=True,
                                fontsize=22, general_data_structure=None):
    ...
    for ci in range(num_cells):
        all_cell_centroid_displacements[ci, :, :] = relative_all_cell_centroids_per_tstep[ci, 1:,
    ...
    for ci in range(all_cell_centroid_displacements.shape[0]):
        ...
        this_cell_persistence_time, this_cell_positive_ts = cu.estimate_persistence_time(T, this_cell_positive_ns,
    ...
    ax.plot(relative_group_centroid_per_tstep[:, 0], relative_group_centroid_per_tstep[:, 1], marker=None,
            label="group centroid", color='k', linewidth=2)
    ...
    ax.set_title(
        "group pers_ratio = {} \n avg. cell pers_ratio = {} (std = {}) \n group pers_time = {} {},  avg. cell pers_time = {} {} (std = {} {})".format(
            group_persistence_ratio, average_cell_persistence_ratio, std_cell_persistence_ratio, group_persistence_time,
            time_unit, average_cell_persistence_time, time_unit, std_cell_persistence_time, time_unit))
    if save_dir != None:
        write_lines_to_file(os.path.join(save_dir, "persistence_data.txt"),
                            ["group peristence ratio: {}\n".format(group_persistence_ratio),
                             "(avg, std) cell persistence ratio: {}, {}\n".format(average_cell_persistence_ratio,
                                                                                  std_cell_persistence_ratio),
                             "(avg, std) cell persistence time: {}, {}".format(average_cell_persistence_time,
    ...
    if make_group_centroid_drift_graph == True:
        general_data_structure = graph_group_centroid_drift(T, time_unit, group_centroid_per_tstep,
                                                            relative_all_cell_centroids_per_tstep, save_dir, save_name,
...
def graph_cell_speed_over_time(num_cells, T, cell_Ls, storefile_path, save_dir=None, save_name=None, max_tstep=None,
                               time_to_average_over_in_minutes=1.0, fontsize=22, general_data_structure=None,
...
def graph_important_cell_variables_over_time(T, L, cell_index, storefile_path, polarity_scores=None, save_dir=None,
                                             save_name=None, max_tstep=None, fontsize=22, general_data_structure=None,
                                             convergence_test=False):
    set_fontsize(fontsize)
    fig, ax = plt.subplots()
    ...
    for data_set, line_style, data_label in zip(
            [sum_rac_act_over_nodes, sum_rho_act_over_nodes, sum_rac_inact_over_nodes, sum_rho_inact_over_nodes,
             rac_cyt_gdi, rho_cyt_gdi], ['b', 'r', 'b--', 'r--', 'c', 'm'],
            ['rac_active', 'rho_active', 'rac_inactive', 'rho_inactive', 'rac_gdi', 'rho_gdi']):
...
def graph_edge_and_areal_strains(T, cell_index, storefile_path, save_dir=None, save_name=None, max_tstep=None,
                                 fontsize=22, general_data_structure=None):
    set_fontsize(fontsize)
    ...
    ax_a.grid(which='both')
    ax_a.set_xlabel("t (min.)")
    avg_areal_strain, std_areal_strain = np.round(np.average(areal_strains), decimals=2), np.round(
        np.std(areal_strains), decimals=2)
...
def graph_rates(T, kgtp_rac_baseline, kgtp_rho_baseline, kdgtp_rac_baseline, kdgtp_rho_baseline, cell_index,
                storefile_path, save_dir=None, save_name=None, max_tstep=None, fontsize=22,
                general_data_structure=None):
    set_fontsize(fontsize)
    fig, ax = plt.subplots()
    ...
    for data_set, line_style, data_label in zip(
            ...
            ['b-.', 'r-.', 'c-.', 'm-.', 'b'],
            ['avg_kgtp_rac ({})'.format(avg_average_kgtp_rac), 'avg_kgtp_rho ({})'.format(avg_average_kgtp_rho),
             'avg_kdgtp_rac ({})'.format(avg_average_kdgtp_rac), 'avg_kdgtp_rho ({})'.format(avg_average_kdgtp_rho),
             'average_coa_signal']):
        ax.plot(time_points, data_set, line_style, label=data_label)
...
def present_collated_single_cell_motion_data(centroids_persistences_speeds_per_repeat, experiment_dir,
                                             total_time_in_hours, time_unit, fontsize=22, general_data_structure=None):
    set_fontsize(fontsize)
    fig, ax = plt.subplots()
    fig_box, ax_box = plt.subplots()
    ...
    if experiment_dir != None:
        write_lines_to_file(os.path.join(experiment_dir, "single_cell_persistence_data.txt"), [
            "P_R over {} hours (mean: {}, std: {})\nP_T mean: {} {}, (std {} {})\ncell x:P_R, P_T".format(
                total_time_in_hours, mean_persistence_ratio, std_persistence_ratio, mean_persistence_time, time_unit,
                std_persistence_time, time_unit)] + ["cell {}: {}, {}\n".format(ci, pr, pt) for ci, pr, pt in
                                                     zip(np.arange(len(persistence_times)), persistence_ratios,
                                                         persistence_times)])
    else:
        ax.set_title(
            "$P_R$ over {} hours (mean: {}, std: {}) \n $P_T$ mean: {} {}, (std {} {})".format(total_time_in_hours,
...
def present_collated_cell_motion_data(time_unit, all_cell_centroids_per_repeat, all_cell_persistence_ratios_per_repeat,
                                      all_cell_persistence_times_per_repeat, all_cell_speeds_per_repeat,
                                      all_cell_protrusion_lifetimes_and_directions_per_repeat,
                                      group_centroid_per_timestep_per_repeat, group_persistence_ratio_per_repeat,
                                      group_persistence_time_per_repeat, experiment_dir, total_time_in_hours,
                                      fontsize=22, chemoattraction_source_coords=None):
    set_fontsize(fontsize)
    max_x_data_lim = 0.0
    min_x_data_lim = 0.0
    max_y_data_lim = 0.0
    ...
    for rpti, all_cell_centroids in enumerate(all_cell_centroids_per_repeat):
        for ci, ccs in enumerate(all_cell_centroids):
            ...
            all_protrusion_lifetimes_and_average_directions = np.append(all_protrusion_lifetimes_and_average_directions,
                                                                        np.array(
                                                                            all_cell_protrusion_lifetimes_and_directions_per_repeat[
                                                                                rpti][ci]), axis=0)

            print("Plotting centroids for cell {}...".format(ci))
            ax_time.plot(relative_ccs[:, 0], relative_ccs[:, 1], marker=None, color=colors.color_list300[ci % 300],
                         alpha=0.5)
    ...
    for rpt_number in range(len(group_centroid_per_timestep_per_repeat)):
        ...
        ax_time.plot(relative_group_centroid_per_timestep[:, 0], relative_group_centroid_per_timestep[:, 1],
                     marker=None, color=colors.color_list300[rpt_number % 300], linewidth=2)
    ...
    if type(chemoattraction_source_coords) != type(None):
        ...
        ax_time.plot([chemoattraction_source_coords[0]], [chemoattraction_source_coords[1]], ls='', marker='.',
    ...
    ax_time.set_title(
        "Experiment over {} hours \n Persistence ratio, cell mean: {} (std: {}), group mean: {} \n Persistence time cell mean: {} {}, (std {} {}), group mean: {} {}".format(
            total_time_in_hours, mean_persistence_ratio, std_persistence_ratio, mean_group_persistence_ratio,
            mean_persistence_time, time_unit, std_persistence_time, time_unit, mean_group_persistence_time, time_unit))
...
def present_collated_group_centroid_drift_data(T, cell_diameter, min_x_centroid_per_tstep_per_repeat,
                                               max_x_centroid_per_tstep_per_repeat,
                                               group_x_centroid_per_tstep_per_repeat, fit_group_x_velocity_per_repeat,
                                               save_dir, total_time_in_hours, fontsize=22, general_data_structure=None,
                                               ax_simple_normalized=None, ax_full_normalized=None, ax_simple=None,
    ...
    for repeat_number in range(num_repeats):
        ...
        if ax_simple_normalized != None:
            ax_simple_normalized.plot(timepoints[:max_timepoint],
                                      normalized_relative_group_centroid_x_coords[:max_timepoint],
                                      color=colors.color_list300[repeat_number % 300])

        if ax_simple != None:
            ax_simple.plot(timepoints[:max_timepoint], relative_group_x_centroid_per_tstep[:max_timepoint],
                           color=colors.color_list300[repeat_number % 300])
            ax_simple.set_ylim([min_ylim, max_ylim])

        if ax_full_normalized != None:
            ax_full_normalized.plot(timepoints[:max_timepoint],
                                    normalized_relative_group_centroid_x_coords[:max_timepoint],
                                    color=colors.color_list300[repeat_number % 300])
            ...
            ax_full_normalized.errorbar(bar_timepoints, bar_points, yerr=[lower_bounds, upper_bounds], ls='', capsize=5,
                                        color=colors.color_list300[repeat_number % 300])

        if ax_full != None:
            ax_full.plot(timepoints[:max_timepoint], relative_group_x_centroid_per_tstep[:max_timepoint],
                         color=colors.color_list300[repeat_number % 300])
            ...
            lower_bounds = np.abs(bar_points - bar_min_points)
            upper_bounds = np.abs(bar_points - bar_max_points)
            ax_full.errorbar(bar_timepoints, bar_points, yerr=[lower_bounds, upper_bounds], ls='', capsize=5,
                             color=colors.color_list300[repeat_number % 300])
    ...
    if plot_speedbox:
        violin = ax_box.violinplot(fit_group_x_velocity_per_repeat, showmedians=True,
    ...
    for fig, base_name, fig_size in zip([fig_simple_normalized, fig_simple, fig_full_normalized, fig_full, fig_box],
                                        ["collated_group_centroid_drift_simple_normalized",
                                         ...
                                         "collated_group_centroid_drift_full", "collated_group_speed_box"],
                                        [(7, 6), (7, 6), (7, 6), (7, 6), (7, 6)]):
        if fig != None:
            show_or_save_fig(fig, fig_size, save_dir, base_name, "")
    #
...
def graph_protrusion_lifetimes_radially(protrusion_lifetime_and_direction_data, num_polar_graph_bins,
                                        total_simulation_time_in_minutes, save_dir=None, save_name=None, fontsize=40,
                                        general_data_structure=None):
    ...
    show_or_save_fig(fig, (14, 12), save_dir, save_name, "B={}".format(num_polar_graph_bins), figtype="png",
                     bbox_inches=None)
    ax.set_yticklabels([])
    show_or_save_fig(fig, (14, 12), save_dir, save_name, "no_labels_B={}".format(num_polar_graph_bins), figtype="png",
                     bbox_inches=None)


# =============================================================================

def graph_protrusion_start_end_causes_radially(protrusion_lifetime_and_direction_data, protrusion_start_end_cause_data,
                                               num_polar_graph_bins, save_dir=None, fontsize=22,
                                               general_data_structure=None):
    set_fontsize(fontsize)
    ...
    for cell_protrusion_data in zip(protrusion_lifetime_and_direction_data, protrusion_start_end_cause_data):
        for protrusion_lifetime_direction_result, protrusion_start_end_cause in zip(cell_protrusion_data[0],
                                                                                    cell_protrusion_data[1]):
...
def graph_forward_backward_protrusions_per_timestep(max_tstep, protrusion_node_index_and_tpoint_start_ends,
                                                    protrusion_lifetime_and_direction_data, T, forward_cones,
    ...
    for cell_protrusion_data in zip(protrusion_node_index_and_tpoint_start_ends,
                                    protrusion_lifetime_and_direction_data):
        for protrusion_start_end_info, protrusion_lifetime_direction_info in zip(cell_protrusion_data[0],
                                                                                 cell_protrusion_data[1]):
...
def graph_coa_variation_test_data_simple(sub_experiment_number, num_timepoints, T, num_cells_to_test, test_coas,
    ...
    for i, n in enumerate(num_cells_to_test):
        for j, tcoa in enumerate(test_coas):
            ax.plot(timepoints, average_cell_group_area_data[i][j],
...
def graph_confinement_data_persistence_ratios(sub_experiment_number, test_num_cells, test_heights,
                                              average_cell_persistence, save_dir=None, general_data_structure=None):
    set_fontsize(fontsize)
    fig, ax = plt.subplots()
...
def graph_confinement_data_persistence_times(sub_experiment_number, test_num_cells, test_heights,
                                             average_cell_persistence, save_dir=None, general_data_structure=None):
    set_fontsize(fontsize)
    fig, ax = plt.subplots()
    ...
    # Add colorbar, make sure to specify tick locations to match desired ticklabels
    show_or_save_fig(fig, (12, 8), save_dir,
                     "confinement_test_graph_persistence_times_{}".format(sub_experiment_number), "")


# ====================================================================

def calculate_statistics_and_write_into_text_file(sub_experiment_number, test_num_cells, test_heights,
                                                  group_persistence_ratios, group_x_velocities, areal_strains,
                                                  save_dir):
    lines = []

    lines.append("(test num cells, test corridor widths): {}".format(str(list(zip(test_num_cells, test_heights)))))
    lines.append("\n")
...
def calculate_statistics_and_write_into_general_file(text_file_name, labels, group_persistence_ratios,
                                                     group_x_velocities, areal_strains, save_dir):
    lines = []
...
def graph_coa_variation_test_data(sub_experiment_number, test_coas, default_cil, corridor_height, num_cells,
    ...
    x_label = "$M_{COA}$\n($M_{CIL}$" + "={}, corridor height = {} cell diam., {} cells)".format(default_cil,
                                                                                                 corridor_height,
                                                                                                 num_cells)
...
def graph_cil_variation_test_data(sub_experiment_number, test_cils, default_coa, corridor_height, num_cells,
    ...
    x_label = "$M_{CIL}$\n($M_{COA}$" + "={}, corridor height = {} cell diam., {} cells)".format(default_coa,
                                                                                                 corridor_height,
                                                                                                 num_cells)
...
def graph_vertex_choice_variation_test_data(test_vertex_choice_ratios_and_randomization_magnitudes, default_cil,
    ...
    x_label = "ratio of randomly selected vertices to total vertices\n$M_{CIL}$" + "={}".format(
        default_cil) + ",($M_{COA}$" + "={}, corridor height = {} cell diam., {} cells)".format(default_coa,
                                                                                                corridor_height,
                                                                                                num_cells)
...
def graph_convergence_test_data(sub_experiment_number, test_num_nodes, cell_speeds, active_racs, active_rhos,
                                inactive_racs, inactive_rhos, special_num_nodes=16, save_dir=None, fontsize=22):
    set_fontsize(fontsize)
...
def graph_specific_convergence_test_data(num_timepoints, T, test_num_nodes, data_per_test_per_repeat_per_timepoint,
    ...
    for i, nn in enumerate(test_num_nodes):
        ...
        for j, marker in enumerate(['.', 'x']):
            ...
            ax.plot(timepoints[:num_data_points], data_per_cell_per_timepoint[j], marker=marker, ls='',
...
def graph_corridor_convergence_test_data(sub_experiment_number, test_num_nodes, cell_full_speeds, active_racs,
                                         active_rhos, inactive_racs, inactive_rhos, special_num_nodes=16, save_dir=None,
                                         fontsize=22):
    set_fontsize(fontsize)
    fig_speed, ax_speed = plt.subplots()
    markers = ['o', 'x']
    ...
    for n in range(2):
        ax_speed.plot(test_num_nodes, [cf[n] for cf in cell_full_speeds], color='g', marker=markers[n],
                      label="{}".format(cell_label[n]), ls='')
    ...
    markers = ['o', 'x']
    for n in range(2):
        ax_rgtpase.plot(test_num_nodes, [cf[n] for cf in active_racs], color='b', marker=markers[n],
                        label="active Rac1: {}".format(cell_label[n]), ls='')
        ax_rgtpase.plot(test_num_nodes, [cf[n] for cf in active_rhos], color='r', marker=markers[n],
                        label="active RhoA: {}".format(cell_label[n]), ls='')
...
def graph_Tr_vs_Tp_test_data(sub_experiment_number, test_Trs, average_cell_persistence_times, save_dir=None,
                             fontsize=22):
    set_fontsize(fontsize)
    fig, ax = plt.subplots()
    ...
    for i in range(num_rows):
        for j in range(num_cols):
            if p < num_experiments:
                present_collated_group_centroid_drift_data(*experiment_drift_args[p], ax_simple=axarr[i, j],
                                                           plot_speedbox=False, plot_if_no_axis_given=False)
...
def graph_nonlin_to_lin_parameter_comparison(kgtp_rac_multipliers, kgtp_rho_multipliers, kgtp_rac_autoact_multipliers,
                                             kgtp_rho_autoact_multipliers, kdgtp_rac_multipliers, kdgtp_rho_multipliers,
                                             kdgtp_rho_mediated_rac_inhib_multipliers,
                                             kdgtp_rac_mediated_rho_inhib_multipliers, save_dir=None):
    ...
    for kgtp_rac, kgtp_rho, kgtp_rac_auto, kgtp_rho_auto, kdgtp_rac, kdgtp_rho, kdgtp_rho_on_rac, kdgtp_rac_on_rho in zip(
...
def graph_chemotaxis_protrusion_lifetimes(sub_experiment_number, test_magnitudes, test_slopes,
                                          protrusion_lifetimes_and_directions_per_magslope, box_width, box_height,
                                          num_cells, save_dir=None, fontsize=22):
    return None
#     set_fontsize(fontsize)
#     assert(len(test_magnitudes) == len(test_slopes))
#     fig, ax = plt.subplots()
